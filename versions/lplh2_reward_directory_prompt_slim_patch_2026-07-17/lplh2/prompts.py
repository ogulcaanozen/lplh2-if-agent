...
- In <setup_commands>, list only commands copied exactly from the supplied recent action and observation pairs that changed the state of the current room and were necessary preparation for the scoring command (for example moving something that hid an exit, or opening something). List them in execution order separated by semicolons. If none, output <setup_commands>none</setup_commands>.
