...
class LPLHAgent:
    ...
    def _resolve_arrival_identity(self, title: str, description: str,
        ...
        if len(registry_signature_matches) > 1:
            ...
            if known_destination in registered_labels:
        ...
        if parse_error or confidence == "low" or decision != "existing":
            label, registry_id = self.kg_map.mint_room(
                title, description, epoch=self.current_epoch, force_new=False
            )
            decision = "new"
        else:
    ...
    def _store_reward_enabler_experiences(self, score_event_key: str,
        ...
        for setup_command in setup_commands or []:
            ...
            if not setup_key or setup_key == scoring_key or setup_key in seen_commands:
                continue
            matching = next(
                (
