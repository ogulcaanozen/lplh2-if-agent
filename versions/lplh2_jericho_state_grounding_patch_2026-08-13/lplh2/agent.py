...
class LPLHAgent:
    ...
    def __init__(self, llm_client: LLMClient = None, fm_client: FmClient = None):
        ...
        self.reward_directory = RewardDirectory()
        self.interaction_stats = InteractionStats()
        self.room_familiarity = RoomFamiliarity()
        ...
        self._room_state_change_steps = {}
        self._direction_block_steps = {}
        self._direction_confirm_steps = {}
        self._last_reopen_audit = []
    ...
    def reset(self, keep_experiences: bool = True):
        ...
        Args:
            keep_experiences: If True, keep the Experience Library, learned
                            situations, and preparation goals across epochs.
                            Epoch-local world/action bookkeeping and situation
                            resolution state are reset. If False, clear all
                            persistent stores.
        """
        ...
        if not keep_experiences:
            ...
            if hasattr(self, "reward_directory"):
                self.reward_directory.full_reset()
            if hasattr(self, "interaction_stats"):
                self.interaction_stats.full_reset()
            if hasattr(self, "room_familiarity"):
                self.room_familiarity.full_reset()
        else:
            if hasattr(self, "reward_directory"):
                self.reward_directory.reset_epoch_flags()
            if hasattr(self, "interaction_stats"):
                self.interaction_stats.reset_epoch()
            if hasattr(self, "room_familiarity"):
                self.room_familiarity.reset_epoch()
    ...
    def act(self, observation: str, score: int, done: bool, info: dict,
        ...
        if self.prev_action is None:
            ...
            self._record_initial_room_familiarity()
        ...
        uncertain_entered_via_at_step_start = (
            self.kg_map.location_entered_dark_via
        )
        uncertain_last_known_at_step_start = self.kg_map.current_location or "unknown"
        uncertain_since_step_at_step_start = (
            self.kg_map.location_uncertain_since_step
        )
        inventory_before = set(self.kg_map.inventory)
        engine_location_available = bool(
            config.ENGINE_LOCATION_GROUNDING
            and info.get("engine_location_available") is True
            and info.get("engine_location_num") is not None
        )
        engine_location_num = None
        if engine_location_available:
            try:
                engine_location_num = int(info.get("engine_location_num"))
            except (TypeError, ValueError):
                engine_location_available = False
        engine_moved = bool(
            engine_location_available
            and self._current_engine_location_num is not None
            and engine_location_num != self._current_engine_location_num
            and not done
        )
        engine_inventory_available = bool(
            config.ENGINE_INVENTORY_GROUNDING
            and info.get("engine_inventory_available") is True
            and not done
        )
        engine_inventory_before = list(self.kg_map.inventory)
        if engine_inventory_available:
            self.kg_map.set_engine_inventory(info.get("engine_inventory", []))
        detail["modules"]["engine_grounding"] = {
            "location_available": engine_location_available,
            "location_num": engine_location_num,
            "location_name": info.get("engine_location_name", ""),
            "enclosure_name": info.get("engine_enclosure_name", ""),
            "location_changed": engine_moved,
            "inventory_available": engine_inventory_available,
            "inventory_before": engine_inventory_before,
            "inventory_after": list(self.kg_map.inventory),
            "inventory_object_count": len(
                info.get("engine_inventory_objects", []) or []
            ),
            "source": "jericho" if (
                engine_location_available or engine_inventory_available
            ) else "llm_fallback",
        }
        engine_location_hint = ""
        if engine_location_available and not done:
            engine_title = (
            ...
            engine_location_hint, _ = self.kg_map.resolve_engine_location(
                engine_location_num,
                title=engine_title,
        ...
        engine_validity_override = None
        if engine_moved and action_valid is not True:
            engine_validity_override = action_valid
            action_valid = True
            if action_split is None:
                try:
                    ...
                    logger.warning("Engine-confirmed action split failed: %s", e)
        ...
        if engine_validity_override is not None:
            detail["modules"]["action_space"]["fm_prev_action_valid"] = (
                engine_validity_override
            )
            detail["modules"]["action_space"]["validity_override"] = {
                "source": "jericho_location_change",
                "reason": "Jericho room object number changed",
            }
        ...
        auxiliary_gate = self._run_auxiliary_gate(
            ...
            current_location_override=engine_location_hint,
        )
        ...
        kg_location_resolution = self._resolve_step_location(
            ...
            reward_change=reward_change,
            engine_location_num=engine_location_num,
            engine_location_name=info.get("engine_location_name", ""),
            engine_enclosure_name=info.get("engine_enclosure_name", ""),
        )
        source_navigation_writes_allowed = (
            engine_location_available
            or self._source_navigation_writes_allowed(
        ...
        if pure_rejection or self.kg_map.location_uncertain:
            ...
            try:
                ...
                if ((action_valid is True or gate_accepted)
                    ...
                    direction_was_confirmed = self.kg_map.was_direction_confirmed(
                        prev_location,
                        movement_direction,
                    )
                    split_key = (
                    ...
                    blocked_message = self.kg_map.blocked_direction_message(
                        prev_location,
                        movement_direction,
                    )
                    direction_key = self._direction_state_key(
                        prev_location,
                        movement_direction,
                    )
                    block_step = int(
                        self._direction_block_steps.get(direction_key, 0) or 0
                    )
                    state_change_step = int(
                        self._room_state_change_steps.get(
                            normalize_location_key(prev_location), 0
                        ) or 0
                    )
                    state_gated_override = bool(
                        blocked_message
                        and self._same_room_state_changed_after(
                            prev_location,
                            block_step,
                        )
                    )
                    ...
                    if blocked_message:
                        blocked_class = (
                            "conditional"
                            if state_gated_override
                            else self.llm.classify_blocked_message(
                                movement_direction,
                                blocked_message,
                            )
                        )
                        kg_location_resolution[
                            "blocked_open_contradiction"
                        ] = {
                            "direction": movement_direction,
                            "message": blocked_message,
                            "classification": blocked_class,
                            "classifier_prompt": (
                                self.llm.last_blocked_classifier_prompt or ""
                            ),
                            "classifier_response": (
                                self.llm.last_blocked_classifier_raw_response
                                or ""
                            ),
                        }
                        if state_gated_override:
                            kg_location_resolution[
                                "blocked_open_contradiction"
                            ]["state_gated_override"] = {
                                "state_change_step": state_change_step,
                    ...
                    room_split = self.kg_map.confirm_direction(
                        ...
                        allow_split=(
                            ...
                            and not engine_location_available
                            and split_key not in self._contradiction_splits_this_epoch
                            and (
                                not blocked_message
                                or blocked_class == "structural"
                            )
                            and not state_gated_override
                        ),
                        preserve_conflict=(
                            config.CONTRADICTION_SPLITTER
                            and not engine_location_available
                    ...
                    if room_split.get("new_sibling"):
                        self._contradiction_splits_this_epoch.add(split_key)
                        kg_location_resolution["room_split"] = room_split
                        prev_location = room_split["new_sibling"]
                        source_state_snapshot = dict(source_state_snapshot or {})
                        source_state_snapshot["location"] = prev_location
                        kg_location_resolution[
                            "previous_location_corrected_to"
                        ] = prev_location
                    self._direction_confirm_steps[direction_key] = self.step_count
                    ...
                    kg_location_resolution["newly_confirmed_exit"] = (
                        not direction_was_confirmed
                    )
                    ...
                    if engine_location_available:
                        ...
                        kg_location_resolution["confirmed_transition"] = transition
                        kg_location_resolution["action_transition_applied"] = True
                        kg_location_resolution["action_transition_status"] = (
                            "engine_action_transition_confirmed"
                        )
                    else:
                        kg_location_resolution["confirmed_transition_type"] = (
                            "action_candidate"
                        )
                        kg_location_resolution["action_transition_candidate"] = transition
        ...
        structural_failure_preserves_exit = False
        failed_direction = self._blocked_direction_for_command(completed_action)
        if (
            action_valid is False
            and failed_direction
            and prev_location
            and self.kg_map.was_direction_confirmed(
                prev_location, failed_direction
            )
            and source_navigation_writes_allowed
            and not done
        ):
            ...
            confirm_step = int(
                self._direction_confirm_steps.get(direction_key, 0) or 0
            )
            state_change_step = int(
                self._room_state_change_steps.get(
                    normalize_location_key(prev_location), 0
            ...
            state_gated_override = self._same_room_state_changed_after(
                prev_location,
                confirm_step,
            )
            blocked_class = (
            ...
            conflict_evidence = {
                ...
                "message": self._clean_text(observation)[:300],
                "classification": blocked_class,
                "classifier_prompt": (
                    self.llm.last_blocked_classifier_prompt or ""
                ),
                "classifier_response": (
                    self.llm.last_blocked_classifier_raw_response or ""
                ),
            }
            if state_gated_override:
                conflict_evidence["state_gated_override"] = {
                    "state_change_step": state_change_step,
                    "confirmed_step": confirm_step,
                }
            kg_location_resolution[
                "confirmed_exit_failure_contradiction"
            ] = conflict_evidence
            if (
                blocked_class == "structural"
                and config.CONTRADICTION_SPLITTER
                and not engine_location_available
                and (
                    prev_location,
                    failed_direction,
                ) not in self._contradiction_splits_this_epoch
            ):
                old_location = prev_location
                room_split = self.kg_map.split_structural_block(
                ...
                if room_split.get("new_sibling"):
                    self._contradiction_splits_this_epoch.add(
                        (old_location, failed_direction)
                    )
                    kg_location_resolution["room_split"] = room_split
                    prev_location = room_split["new_sibling"]
                    source_state_snapshot = dict(source_state_snapshot or {})
                    source_state_snapshot["location"] = prev_location
                    kg_location_resolution["previous_location_corrected_to"] = (
                        prev_location
                    )
                    kg_location_resolution["location_after_update"] = (
                        prev_location
                    )
            elif blocked_class == "conditional":
                structural_failure_preserves_exit = True

        timer = time.perf_counter()
        if engine_location_available:
            action_transition_result = {
                "status": "engine_location_grounded",
                "candidate": kg_location_resolution.get(
                    "action_transition_candidate", {}
                ),
                "decision": {},
                "applied": bool(
                    kg_location_resolution.get("action_transition_applied")
                ),
                "reason": "Jericho room identity is authoritative",
            }
        elif pure_rejection:
            action_transition_result = {
        ...
        if action_transition_result.get("applied"):
            ...
            kg_location_resolution["newly_confirmed_exit"] = bool(
                action_transition_result.get("newly_confirmed_exit")
            )
            kg_location_resolution["location_after_update"] = self.kg_map.current_location
        ...
        if engine_inventory_available:
            inventory_reconciliation = {
                "module": "inventory_reconciliation",
                "status": "engine_grounded",
                "source": "jericho",
                "applied": engine_inventory_before != list(self.kg_map.inventory),
                "before": engine_inventory_before,
                "after": list(self.kg_map.inventory),
                "authoritative": True,
                "reason": "Jericho get_inventory() supplied the carried items",
            }
        elif pure_rejection:
        ...
        arrival_world_state_force_reason = (
            self._arrival_world_state_force_reason(
                previous_location=prev_location,
                visited_rooms_before=visited_rooms_before,
                location_resolution=kg_location_resolution,
            )
        ...
        if pure_rejection:
            ...
            world_state_extraction = self._apply_gate_world_state_update(
                auxiliary_gate=auxiliary_gate,
                force_reason=arrival_world_state_force_reason,
            )
        ...
        same_room_state_change = bool(
            normalize_location_key(prev_location)
            == normalize_location_key(self.kg_map.current_location or "")
            and (
                inventory_reconciliation.get("applied")
                or world_state_extraction.get("applied")
                or environmental_change_detection.get("environmental_change")
            )
        )
        if same_room_state_change and prev_location:
            self._room_state_change_steps[
                normalize_location_key(prev_location)
            ] = self.step_count
            kg_location_resolution["same_room_state_change_step"] = (
                self.step_count
            )
        ...
        situation_detail["persistent_situations"] = (
            self.situation_memory.persistent_situations()
        )
        situation_detail["resolved_this_epoch"] = (
            self.situation_memory.resolved_this_epoch()
        )
        ...
        if experience_triggered:
            try:
                history_text = (
                    self._format_death_history(uncertain_since_step_at_step_start)
                    if is_death else self._format_history()
                )
                score_location = self._first_known_location(
                    source_generation.get("location_at_generation"),
                    prev_location,
                    self.kg_map.current_location,
                    "Starting Location",
                )
                ...
                if is_death:
                    ...
                    precomputed_loss_summary = self.llm.summarize_loss_experience(
                        ...
                        death_context=self._death_uncertainty_context(
                            was_location_uncertain_at_step_start,
                            uncertain_entered_via_at_step_start,
                            uncertain_last_known_at_step_start,
                            uncertain_since_step_at_step_start,
                        ),
                    )
                    ...
                    goal_hazard = self._goal_hazard_context(
                        ...
                        was_location_uncertain=was_location_uncertain_at_step_start,
                        uncertain_entered_via=uncertain_entered_via_at_step_start,
                        uncertain_last_known=uncertain_last_known_at_step_start,
                    )
                ...
                if event_key and self.experience_lib.event_seen(event_key):
                    ...
                    if is_death:
                        ...
                        if reward_change > 0:
                            ...
                            setup_commands = self._parse_setup_commands(
                                exp_summary,
                                scoring_action=completed_action,
                            )
                ...
                if (
                    reward_change > 0
                    and score_event_key
                    and not self._is_provisional_location(score_location)
                ):
                ...
                if is_death:
                    ...
                    goal_transition, hypothesis_log = (
                        self._handle_death_goal_lifecycle(
                            ...
                            death_summary=precomputed_loss_summary,
                        )
                    )
                    if goal_transition:
                        ...
                        death_visible_objects = self._visible_objects_for_location(
                            self._effective_location()
                        )
                        situation_detail["active_situations_after"] = (
                            self.situation_memory.active_situations(
                                inventory=list(self.kg_map.inventory),
                                visible_objects=death_visible_objects,
                            )
                        )
                    if hypothesis_log:
        ...
        if (
            reward_change == 0
            and not done
            and not pure_rejection
            and not self.kg_map.location_uncertain
        ):
            ...
            for trigger_type, trigger_meta in neutral_triggers:
                try:
                    ...
                    neutral_source_location = (
                        neutral_issued_location
                        if trigger_type == "navigation"
                        else ""
                    )
                    neutral_source_registry_id = (
                        neutral_location_registry_id
                        if trigger_type == "navigation"
                        else ""
                    )
                    ...
                    if self._is_empty_experience_summary(summary):
                        self.experience_lib.record_neutral_event(
                            ...
                            metadata={
                                ...
                                "source_location": neutral_source_location,
                                "source_registry_id": neutral_source_registry_id,
                                "prev_location": trigger_meta.get("prev_location"),
        ...
        if location_changed_for_affordance:
            ...
            self.kg_map.mark_direction_tried_at(
                ...
                failure_location,
                message=observation,
                preserve_confirmed=structural_failure_preserves_exit,
            )
            self._direction_block_steps[
                self._direction_state_key(
                    failure_location,
                    completed_direction,
                )
            ] = self.step_count
        ...
        world_fingerprint_at_generation = source_generation.get(
            "world_fingerprint_at_generation"
        )
        if not world_fingerprint_at_generation:
            world_fingerprint_at_generation = self._world_fingerprint(
                location=source_memory_location,
                inventory=sorted(inventory_before),
                visible_objects_with_state=(
                    self._visible_objects_with_state_for_location(
                        source_memory_location
                    )
                ),
            )
        attempt_ledger_detail = self.attempt_ledger.record_step(
            ...
            step=self.step_count,
            epoch=self.current_epoch,
            world_fingerprint=world_fingerprint_at_generation,
        )
        ...
        if attempt_ledger_detail.get("status") == "recorded":
            if (
                location_changed_for_affordance
                and normalize_location_key(source_memory_location)
                != normalize_location_key(self.kg_map.current_location or "")
            ):
                self._epoch_path.append((
                    source_memory_location,
                    completed_action,
                    self.kg_map.current_location or "unknown",
                ))
        ...
        route_hop_feedback = []
        for hop in self.reward_directory.matching_hops(
            source_memory_location,
            completed_action,
            self.earned_score_event_keys_this_epoch,
        ):
            if location_changed_for_affordance:
                cleared = self.reward_directory.clear_hop_failure(
                    hop["event_key"],
                    hop["source"],
                    hop["command"],
                )
                route_hop_feedback.append({
                    **hop,
                    "status": (
                        "failure_cleared_after_success"
                        if cleared else "hop_succeeded"
                    ),
                })
            else:
                self.reward_directory.flag_hop_failure(
                    hop["event_key"],
                    hop["source"],
                    hop["command"],
                )
                route_hop_feedback.append({
                    **hop,
                    "status": "failure_flagged",
                })
        detail["modules"]["reward_route_feedback"] = route_hop_feedback

        detail["modules"]["room_familiarity"] = (
            self._update_room_familiarity(
                location_changed=location_changed_for_affordance,
                source_location=source_memory_location,
                reward_change=reward_change,
                outcome_class=attempt_ledger_detail.get(
                    "outcome_class", ""
                ),
                situation_detail=situation_detail,
                kg_location_resolution=kg_location_resolution,
            )
        )
        detail["modules"]["reward_directory"] = {
            "entries": self.reward_directory.entries(),
            "route_hop_feedback": route_hop_feedback,
        }
        detail["modules"]["kg_map"]["kg_map_context"] = (
            self._prompt_kg_map_context()
        )
    ...
    def _initialize_kg_from_observation(self, observation: str, info: dict = None):
        ...
        try:
            ...
            triples = []
            try:
                triples = self.fm.extract_relations(
                    "look", observation, max_new=192
                )
            except Exception as e:
                logger.warning("Initial relation extraction failed: %s", e)
            room_title = ""
            if config.AUX_GATE_LOCATION_VERDICT and not engine_location_available:
                try:
                    response = self.llm.gate_auxiliary_modules(
                        ...
                        inventory_before=list(self.kg_map.inventory),
                        inventory=list(self.kg_map.inventory),
                        visible_objects=[],
                        active_situations=[],
            ...
            if engine_location_available:
                room_title, _ = self.kg_map.resolve_engine_location(
                    info.get("engine_location_num"),
                    title=room_title,
                    engine_name=info.get("engine_location_name", ""),
                    observation=probe_text or observation,
                    epoch=self.current_epoch,
                )
                self._current_engine_location_num = int(
                    info.get("engine_location_num")
                )
            else:
                if not room_title:
    ...
    def _effective_location(self) -> str:
        """Prompt/memory location, including an unregistered unseen-area label."""
        if self.kg_map.location_uncertain:
            return self.kg_map.uncertain_location_label()
        return self.kg_map.current_location or "unknown"

    @staticmethod
    def _is_provisional_location(location: str) -> bool:
        """Synthetic unseen-area labels are prompt-local, not persistent rooms."""
        return normalize_location_key(location).startswith("unseen area via ")

    def _resolve_step_location(self, auxiliary_gate: dict, action: str,
                               observation: str, look_probe_text: str,
                               previous_location: str, done: bool,
                               reward_change: int = 0,
                               engine_location_num=None,
                               engine_location_name: str = "",
                               engine_enclosure_name: str = "") -> dict:
        """Resolve one arrival, preferring Jericho identity when available."""
        if config.ENGINE_LOCATION_GROUNDING and engine_location_num is not None:
            return self._resolve_engine_step_location(
                auxiliary_gate=auxiliary_gate,
                action=action,
                observation=observation,
                ...
                engine_location_num=engine_location_num,
                engine_location_name=engine_location_name,
                engine_enclosure_name=engine_enclosure_name,
            )
        was_location_uncertain = bool(self.kg_map.location_uncertain)
        ...
        command_status = self._clean_text(
            decision.get("command_outcome", {}).get("status", "")
        ).lower()
        if (
            config.MOVEMENT_VERDICT_OVERRIDE
            and direction
            ...
            registry_id = self.kg_map.registry_id_for(
                previous_location or self.kg_map.current_location or ""
            )
            registry_entry = self.kg_map.room_registry.get(registry_id, {})
            arrival_signature = self.kg_map.description_signature(
                signature_title,
                look_probe_text or observation,
            )
            stored_signatures = self.kg_map.entry_signatures(registry_entry)
            if arrival_signature and stored_signatures:
        ...
        result = {
            ...
            "location_uncertain": bool(self.kg_map.location_uncertain),
            "uncertainty_event": "",
        ...
        if done:
            ...
            return result
        title_observed_in_game_text = bool(grounded or fallback_title)
        if was_location_uncertain and (
            not selected_title
            or not title_observed_in_game_text
        ):
            result["location_uncertain"] = True
            result["uncertainty_event"] = "uncertainty_retained"
            result["uncertainty_evidence"] = (
                "an unseen destination can be resolved only by a room title "
                "present in the current observation or look probe"
            )
            result["action_transition_status"] = (
                "uncertain_location_not_cleared"
            )
            return result
        if gate_ok and moved == "yes" and not selected_title:
            self.kg_map.set_location_uncertain(
                True,
                entered_via=action,
            ...
            result["uncertainty_event"] = "uncertainty_retained"
            result["uncertainty_evidence"] = (
                "movement was reported but no destination title was grounded "
                "in the observation or look probe"
            )
        ...
        if (
            ...
            and not raw_title
            and not grounded
            and not has_positive_movement_evidence
        ):
            result["chosen_location_hint"] = self.kg_map.current_location or ""
            result["location_after_update"] = self.kg_map.current_location
            result["action_transition_status"] = (
                "titleless_unclear_nonmovement_retained"
            )
            result["resolver_skipped_reason"] = (
                "no observed title or positive movement evidence"
            )
            return result
        ...
        result["location_uncertain"] = bool(self.kg_map.location_uncertain)
        if not result["location_uncertain"]:
            result["uncertainty_event"] = "uncertainty_resolved"
            result["uncertainty_evidence"] = (
                grounded_title or probe_title or selected_title
            )
    ...
    def _resolve_engine_step_location(
        self,
        auxiliary_gate: dict,
        action: str,
        observation: str,
        ...
        engine_location_num,
        engine_location_name: str,
        engine_enclosure_name: str,
    ) -> dict:
        """Use Jericho identity as truth while retaining readable room labels."""
        try:
            engine_location_num = int(engine_location_num)
        except (TypeError, ValueError):
            engine_location_num = None
        prior_engine_num = self._current_engine_location_num
        engine_moved = bool(
            engine_location_num is not None
            and prior_engine_num is not None
            and engine_location_num != prior_engine_num
        )
        raw_title = (
        ...
        result = {
            ...
            "title_overrode_fm": False,
            "chosen_location_hint": previous_location or "",
            "previous_location": previous_location,
            "location_after_update": self.kg_map.current_location,
            "confirmed_transition_type": "",
            "confirmed_transition": {},
            ...
            "action_transition_gate_decision": {},
            "action_transition_applied": False,
            "action_transition_status": "engine_no_move",
            "gate_location_verdict": copy.deepcopy(
                (auxiliary_gate or {}).get("decision", {}).get(
                    "location_verdict", {}
                )
            ),
            "verdict_validated": True,
            "probe_title": self._extract_probe_room_title(look_probe_text),
            "resolver_invoked": False,
            "resolver_decision": "engine_identity",
            "resolver_confidence": "authoritative",
            "registry_id": self.kg_map.registry_id_for(previous_location or ""),
            "resolution_mode": "jericho_engine",
            "location_uncertain": False,
            "uncertainty_event": "",
            "uncertainty_evidence": "Jericho supplied a stable room object number",
            "room_split": {},
            "use_legacy_location_pipeline": False,
            "movement_override": {},
            "resolver_skipped_reason": "Jericho room identity available",
            "engine_location_num": engine_location_num,
            "engine_location_name": str(engine_location_name or ""),
            "engine_enclosure_name": str(engine_enclosure_name or ""),
            "engine_previous_location_num": prior_engine_num,
            "engine_location_changed": engine_moved,
        }
        if done or engine_location_num is None:
            result["action_transition_status"] = "terminal_location_not_applied"
            return result

        description = look_probe_text or observation
        resolved, registry_id = self.kg_map.resolve_engine_location(
            engine_location_num,
            title=raw_title,
            engine_name=engine_location_name,
            observation=description,
            epoch=self.current_epoch,
        )
        if not resolved:
            result["resolver_confidence"] = "unavailable"
            result["action_transition_status"] = "engine_resolution_failed"
            return result

        if engine_moved or not self.kg_map.current_location:
            self.kg_map.confirm_arrival(
                resolved,
                observation=description,
                from_location=previous_location if engine_moved else "",
                action=action if engine_moved else "look",
            )
        elif self.kg_map.current_location != resolved:
            self.kg_map.confirm_arrival(
                resolved,
                observation=description,
                from_location="",
            ...
            result["engine_cursor_corrected"] = True

        self._current_engine_location_num = engine_location_num
        result.update({
            "chosen_location_hint": resolved,
            "location_after_update": resolved,
            "registry_id": registry_id,
            "action_transition_status": (
                "engine_move_confirmed" if engine_moved else "engine_no_move"
            ),
            "location_uncertain": False,
        })
        if engine_moved and previous_location and previous_location != resolved:
            result["confirmed_transition_type"] = "engine"
            result["confirmed_transition"] = {
    ...
    def _resolve_arrival_identity(self, title: str, description: str,
        ...
        registry_signature_matches = [
            (registry_id, entry)
            for registry_id, entry in self.kg_map.room_registry.items()
            if (signature
                and self.kg_map._base_location_key(entry.get("base", ""))
                == self.kg_map._base_location_key(title)
                and signature in self.kg_map.entry_signatures(entry))
        ]

        # Identical arrival text cannot be distinguished by another LLM call.
        # Truly identical physical siblings are separated later by contradictory
        # traversal evidence, not by guessing at arrival time.
        if len(registry_signature_matches) == 1:
            registry_id, _ = registry_signature_matches[0]
            label = self.kg_map.adopt_registry_room(
                registry_id, observation=description
            )
            return label, {
                **log,
                "resolver_decision": "signature_match",
        ...
        if len(registry_signature_matches) > 1:
            registered_labels = [
                entry.get("label", "")
                for _, entry in registry_signature_matches
                if entry.get("label", "")
            ]
            current_matches = [
                label for label in candidates if label in registered_labels
            ]
            known_destination = self.kg_map._destination_for_known_edge(
                from_location, action, title
            )
            arrival_registry_id, arrival_label = (
                self.kg_map.arrival_edge_match(
                    registry_signature_matches,
                    from_location,
                    action,
                )
            )
            if arrival_registry_id and arrival_label in registered_labels:
                label = arrival_label
                preference = "arrival_edge"
            elif known_destination in registered_labels:
                label = known_destination
                preference = "known_edge"
            elif current_matches:
                label = current_matches[0]
                preference = "most_recent_visit"
            else:
                label = registered_labels[0]
                preference = "registry_order"
            registry_id = arrival_registry_id or next(
                rid for rid, entry in registry_signature_matches
            ...
            return label, {
                **log,
                "resolver_decision": "signature_match_ambiguous",
                "resolver_confidence": "high",
        ...
        if cached:
            ...
            cached_registry_id = cached.get("registry_id", "")
            if cached_registry_id:
                cached_label = self.kg_map.adopt_registry_room(
                    cached_registry_id, observation=description
                ) or cached_label
            return cached_label, {
                **log, **cached["log"], "resolver_cached": True
            }

        cards = (
            self.kg_map.candidate_cards(title, limit=4)
            if candidates
            else self.kg_map.registry_candidate_cards(title, limit=4)
        )
        log["candidate_source"] = "epoch_nodes" if candidates else "registry"
        ...
        for _ in range(2):
            try:
                body = self.llm.resolve_location_identity(
                    ...
                    agent_inventory=list(self.kg_map.inventory),
                )
        ...
        if parse_error or confidence == "low" or decision != "existing":
            ...
            if registry_id:
                ...
                log["signature_alias_added"] = self.kg_map.add_signature_alias(
                    registry_id, signature
                )
        log["resolver_decision"] = decision
        log["resolver_confidence"] = confidence or "low"
        log["registry_id"] = registry_id
        if parse_error:
            log["resolver_error"] = parse_error
        self._location_resolver_cache[cache_key] = {
            "label": label,
            "registry_id": registry_id,
            "log": dict(log),
        }
    ...
    def _generate_command(self, observation: str, score: int,
        ...
        current_objects = self._visible_objects_for_location(
            current_location
        )
        current_objects_with_state = self._visible_objects_with_state_for_location(
            current_location
        )
        current_world_fingerprint = self._world_fingerprint(
            location=current_location,
            inventory=list(self.kg_map.inventory),
            visible_objects_with_state=current_objects_with_state,
        )
        ...
        tried_here_context = self._tried_here_context(
            current_location,
            current_world_fingerprint,
            current_state_snapshot,
        )
        ...
        stored_situations = self.situation_memory.format_for_prompt(
            inventory=list(self.kg_map.inventory),
            visible_objects=current_objects_with_state or current_objects,
        )
        room_danger_history = self._room_danger_history_for_prompt(
            current_location
        )
        action_space_context = self.action_space.to_prompt_string(current_objects)
        reward_navigation_edges = {
            location: dict(node.get("direction", {}) or {})
            for location, node in self.kg_map.nodes.items()
        }
        route_hop = self.reward_directory.next_hop_from(
            current_location,
            self.earned_score_event_keys_this_epoch,
            extra_edges=reward_navigation_edges,
        )
        route_guidance = (
            f"From here, '{route_hop['command']}' continues the recorded "
            f"route toward +{route_hop['points']} at "
            f"{route_hop['entry_room']}."
            if route_hop else "none"
        )
        known_rewards_context = self.reward_directory.render(
            self.earned_score_event_keys_this_epoch,
            current_location=current_location,
            extra_edges=reward_navigation_edges,
        )
        nearby_side_rewards = self.reward_directory.nearby_side_rewards(
            current_location,
            self.earned_score_event_keys_this_epoch,
            max_hops=2,
            extra_edges=reward_navigation_edges,
        )
        if nearby_side_rewards:
            side_lines = []
            for item in nearby_side_rewards:
                hop_note = (
                    "here"
                    if int(item.get("hop_count", 0)) == 0
                    else (
                        f"{item.get('hop_count')} hop(s), first go "
                        f"{item.get('first_command')}"
                    )
                )
                side_lines.append(
                    f"- {item.get('scoring_command')} at "
                    f"{item.get('room')} (+{item.get('points')}, {hop_note})"
                )
            known_rewards_context += (
                "\nside rewards within 2 hops:\n"
                + "\n".join(side_lines)
            )
        untouched_objects = self.interaction_stats.untouched_objects(
            self.kg_map.registry_id_for(
                current_location
            ),
            current_objects_with_state or current_objects,
        )
        object_history_notes = self.interaction_stats.notes_for_visible_objects(
            self.kg_map.registry_id_for(
                current_location
            ),
            current_objects_with_state or current_objects,
        ) or "none"
        relevant_lessons = self._relevant_lessons_for_brainstorm()
        if initial_generation:
        ...
        affordance_result = self._brainstorm_affordances(
            ...
            tried_here=tried_here_context,
            relevant_lessons=relevant_lessons,
            known_rewards=known_rewards_context,
            route_guidance=route_guidance,
            untouched_objects=untouched_objects,
            object_history_notes=object_history_notes,
            room_danger_history=room_danger_history,
            score=score,
        ...
        recent_path = self._recent_path_for_prompt()
        ...
        prompt = LPLH_ACTION_GENERATION_PROMPT.format(
            ...
            history=self._format_history(limit=6),
            history_length=6,
            score=score,
            observation=observation,
        )
        ...
        try:
            ...
            agenda_selection_audit = self._agenda_selection_audit(
                command,
                affordance_result.get("affordance_agenda", []),
            )
            acquisition_choice_audit = self._acquisition_choice_audit(
                command,
                agenda_selection_audit,
                observation,
            )
            if initial_generation:
        ...
        generation = {
            ...
            "known_rewards_context": known_rewards_context,
            "route_guidance": route_guidance,
            "nearby_side_rewards": nearby_side_rewards,
            "untouched_objects": untouched_objects,
            "object_futility_notes": object_history_notes,
            "brainstormed_command_ideas": brainstormed_command_ideas,
            ...
            "tried_here_context": tried_here_context,
            "relevant_lessons": relevant_lessons,
            "recent_path": recent_path,
            "state_snapshot_at_generation": current_state_snapshot,
            "state_key_at_generation": current_state_key,
            "world_fingerprint_at_generation": current_world_fingerprint,
            "location_at_generation": current_location,
            ...
            "command_reopen_audit": list(self._last_reopen_audit),
            "repeat_self_check": repeat_check,
            "agenda_selection_audit": agenda_selection_audit,
            "acquisition_choice_audit": acquisition_choice_audit,
    ...
    def _brainstorm_affordances(self, observation: str, current_objects: list,
                                ...
                                tried_here: str,
                                relevant_lessons: str,
                                known_rewards: str,
                                route_guidance: str,
                                untouched_objects: list,
                                object_history_notes: str,
                                score: int,
                                room_danger_history: str = "none recorded for current room",
                                reset_cache: bool = False,
                                gate_decision: dict = None) -> dict:
        """Run LPLH2 affordance brainstorming for the next action prompt."""
        location = self._effective_location()
        known_failed_here = self.failed_action_memory.format_for_prompt(
            location
        )
        ...
        same_state_tried_commands = self.state_action_memory.commands_for_state(
            self._repetition_state_snapshot(
                location=location,
                observation=observation,
                visible_objects=current_objects,
                inventory=list(self.kg_map.inventory),
        ...
        object_tiers_here = self.interaction_stats.tiers_for_objects(
        ...
        result = {
            ...
            "known_failed_commands_here": known_failed_here,
            "problem_attempts_here": problem_attempts_here,
            "tried_here": tried_here,
            "relevant_lessons": relevant_lessons,
            "known_rewards": known_rewards,
            "route_guidance": route_guidance,
            "untouched_objects": list(untouched_objects or []),
            "object_history_notes": object_history_notes,
            "room_danger_history": room_danger_history,
            ...
            "object_tiers_here": object_tiers_here,
            "pending_carryover_commands": [],
            "dropped_carryover_commands": [],
            "agenda_items_already_carried": [],
        ...
        result["dropped_carryover_commands"] = list(
            self.affordance_brainstormer.last_dropped_carryover
        )
        ...
        if not should_run:
            ...
            if result["ideas"]:
                result["affordance_agenda"] = self.affordance_brainstormer.build_agenda(
                    ...
                    object_tiers=object_tiers_here,
                )
                result["preparation_validations"] = list(
                    self.affordance_brainstormer.last_preparation_validations
                )
                result["agenda_deduplication"] = list(
                    self.affordance_brainstormer.last_agenda_deduplication
                )
                result["agenda_items_already_carried"] = list(
                    self.affordance_brainstormer.last_inventory_completions
                )
        ...
        try:
            response_body = self.llm.brainstorm_affordances(
                ...
                recent_command_outcomes=[
                    ...
                    for item in recent_command_outcomes[-4:]
                ],
                ...
                visit_advisory=self._visit_advisory_context(
                    result["active_situations"]
                ),
                relevant_lessons=relevant_lessons,
                known_rewards=known_rewards,
                route_guidance=route_guidance,
    ...
    def _run_auxiliary_gate(self, action: str, observation: str, done: bool,
                            ...
                            look_probe_text: str = "",
                            current_location_override: str = "") -> dict:
        """Use one aux LLM call to route selected expensive helper modules."""
        location = current_location_override or self._effective_location()
        ...
        active_situations_for_affordance = self.situation_memory.active_situations(
            inventory=list(self.kg_map.inventory),
            visible_objects=visible_objects,
        )
        ...
        result = {
            ...
            "inventory": list(self.kg_map.inventory),
            "active_situations": active_situations_for_affordance,
            "recent_failed_commands": recent_failed_for_gate,
            "known_failed_commands_here": known_failed_here,
            "problem_attempts_here": problem_attempts_here,
            "recent_command_outcomes": recent_command_outcomes,
    ...
    def _apply_gate_inventory_update(self, auxiliary_gate: dict,
        ...
        try:
            ...
            if not update.get("changed"):
                ...
                return result
            if (
                update.get("authoritative")
                and not update.get("items_now_carried")
                and not self._authoritative_empty_inventory_is_grounded(
                    update,
                    (auxiliary_gate or {}).get("observation", ""),
                )
            ):
                result["status"] = "skipped_uncorroborated_empty_authoritative"
                result["reason"] = (
                    "An authoritative empty inventory requires an exact "
                    "observation quote that says nothing is carried."
                )
                return result
    ...
    def _authoritative_empty_inventory_is_grounded(
        self,
        update: dict,
        observation: str,
    ) -> bool:
        """Require explicit game text before clearing the complete inventory."""
        quote = self._clean_text((update or {}).get("evidence_quote", ""))
        observation_text = self._clean_text(observation)
        if not quote or quote.lower() not in observation_text.lower():
            return False
        normalized = normalize_command_key(quote)
        ...
        return any(phrase in normalized for phrase in empty_inventory_phrases)
    ...
    def _apply_gate_world_state_update(
            self, auxiliary_gate: dict, force_reason: str = "") -> dict:
        ...
        result = {
            ...
            "forced_reason": self._clean_text(force_reason),
        }
        should_run = (
            bool(route.get("run")) if isinstance(route, dict) else False
        ) or bool(force_reason)
        if not should_run:
            result["reason"] = self._clean_text(route.get("reason", "")) if isinstance(route, dict) else ""
            return result
        try:
            ...
            if force_reason:
                gate_reason = (
                    f"forced:{force_reason}; gate:{gate_reason or 'not routed'}"
                )
    ...
    def _arrival_world_state_force_reason(
            self, previous_location: str, visited_rooms_before: set,
            location_resolution: dict) -> str:
        """Force semantic room-object extraction only on meaningful arrivals."""
        if self.kg_map.location_uncertain:
            return ""
        current_location = self.kg_map.current_location or ""
        if (
            not current_location
            or normalize_location_key(current_location)
            == normalize_location_key(previous_location)
        ):
            return ""
        visited_keys = {
            normalize_location_key(location)
            for location in (visited_rooms_before or set())
        }
        if normalize_location_key(current_location) not in visited_keys:
            return "first_confirmed_room_visit"
        if location_resolution.get("signature_alias_added"):
            return "meaningfully_new_arrival_description"
        return ""
    ...
    def _apply_gate_action_transition(
        ...
        existing_destination = (
            self.kg_map.nodes.get(source, {})
            .get("confirmed_actions", {})
            .get(action_key, "")
        )
        self.kg_map.confirm_action_transition(
        ...
        result["newly_confirmed_exit"] = not bool(existing_destination)
        return result
    ...
    def _normalize_gate_inventory_update(self, value) -> dict:
        if not isinstance(value, dict):
            value = {}
        original_changed = self._coerce_gate_bool(
        ...
        items_now_carried = self._clean_gate_item_list(
            value.get("items_now_carried", [])
        )
        ...
        return {
            ...
            "schema_repaired": effective_changed and not original_changed,
            "authoritative": authoritative,
            "items_now_carried": items_now_carried,
            "items_added": items_added,
            "items_removed": items_removed,
            "evidence_quote": self._clean_text(value.get("evidence_quote", "")),
            "reason": self._clean_text(value.get("reason", "")),
        }
    ...
    def _skipped_situation_detection_result(self, action: str,
        ...
        persistent_before = (
            self.situation_memory.persistent_situations_for_prompt()
        )
        return {
            ...
            "persistent_situations_before": persistent_before,
    ...
    def _interaction_target_for_command(self, command: str,
                                        source_generation: dict,
                                        action_split,
                                        inventory_before: set) -> dict:
        """Ground an executed object command even when FM rejected its syntax."""
        candidates: list[str] = []
        ...
        def add_candidate(value):
            ...
            if clean and key and key not in {
                normalize_text(item) for item in candidates
            }:
                candidates.append(clean)

        source_generation = source_generation or {}
        brainstorm = source_generation.get("affordance_brainstorming", {}) or {}
        snapshot = source_generation.get("state_snapshot_at_generation", {}) or {}
        for value in snapshot.get("visible_objects", []) or []:
            add_candidate(value)
        for value in brainstorm.get("visible_object_names", []) or []:
            add_candidate(value)
        for value in brainstorm.get("visible_objects", []) or []:
            add_candidate(value)
        for value in inventory_before or set():
            add_candidate(value)

        audit = source_generation.get("agenda_selection_audit", {}) or {}
        agenda_target = self._clean_text(audit.get("selected_target_object", ""))
        if agenda_target and audit.get("selected_target_grounded"):
            return {
                "object_noun": agenda_target.lower(),
                "source": "selected_agenda_target",
                "candidates": candidates,
            }

        split_objects = (
            action_split.get("objects", [])
            if isinstance(action_split, dict) else []
        )
        if split_objects:
            return {
                "object_noun": self._clean_text(split_objects[0]).lower(),
                "source": "fm_action_split",
                "candidates": candidates,
            }
        ...
        grounded = [
            ...
            and f" {normalize_text(item)} " in f" {command_key} "
        ]
        if grounded:
            target = max(
                grounded,
            ...
            return {
                "object_noun": target.lower(),
                "source": "grounded_visible_or_inventory_fallback",
                "candidates": candidates,
            }
        return {
            "object_noun": "",
    ...
    def _agenda_selection_audit(self, command: str,
        ...
        """Record agenda compliance without changing the selected command."""
        selected = None
        grounded_before = []
        all_grounded = []
        for rank, entry in enumerate(affordance_agenda or [], start=1):
            pending = list(entry.get("pending_commands", []) or [])
            if entry.get("target_grounded"):
                all_grounded.extend({
                    "rank": rank,
                    "command": item,
                    "target_object": entry.get("target_object", ""),
                } for item in pending)
            for position, pending_command in enumerate(pending, start=1):
                if commands_equivalent(command, pending_command):
                    selected = {
                        "rank": rank,
                        "command_position": position,
                        "agenda_command": pending_command,
                        "target_object": entry.get("target_object", ""),
                        "target_grounded": bool(entry.get("target_grounded")),
                        "agenda_type": entry.get("agenda_type", ""),
                        "preparation_for": entry.get("preparation_for", ""),
                        "preparation_relation": entry.get(
                            "preparation_relation", ""
                        ),
                        "agenda_reason": entry.get("reason", ""),
                        "object_tier": entry.get("object_tier", "FRESH"),
                        "earlier_commands_in_entry": pending[:position - 1],
                    }
                    break
            if selected:
                break
            if entry.get("target_grounded"):
                grounded_before.extend({
                    "rank": rank,
                    "command": item,
                    "target_object": entry.get("target_object", ""),
                } for item in pending)
        return {
            "selected_from_agenda": bool(selected),
            "selected_rank": selected.get("rank") if selected else None,
            "selected_command_position": (
                selected.get("command_position") if selected else None
            ),
            "selected_agenda_command": (
                selected.get("agenda_command", "") if selected else ""
            ),
            "selected_target_object": (
                selected.get("target_object", "") if selected else ""
            ),
            "selected_target_grounded": bool(
                selected and selected.get("target_grounded")
            ),
            "selected_agenda_type": (
                selected.get("agenda_type", "") if selected else ""
            ),
            "selected_preparation_for": (
                selected.get("preparation_for", "") if selected else ""
            ),
            "selected_preparation_relation": (
                selected.get("preparation_relation", "") if selected else ""
            ),
            "selected_agenda_reason": (
                selected.get("agenda_reason", "") if selected else ""
            ),
            "selected_object_tier": (
                selected.get("object_tier", "") if selected else ""
            ),
            "earlier_commands_in_selected_entry": (
                selected.get("earlier_commands_in_entry", []) if selected else []
            ),
            "higher_ranked_grounded_pending": grounded_before,
            "grounded_pending_when_not_selected": (
                all_grounded if not selected else []
            ),
        }

    def _acquisition_choice_audit(self, command: str,
                                  agenda_audit: dict,
                                  observation: str) -> dict:
        """Log why an acquisition command was selected without changing it."""
        command_key = normalize_command_key(command)
        match = re.match(
            r"^(?:take|get|grab|acquire|collect)\s+(.+)$|^pick\s+up\s+(.+)$",
            command_key,
        )
        target = ""
        if match:
        ...
        return {
            "is_acquisition": bool(match),
            "command": command,
            "target": target,
            "selected_from_agenda": bool(
                (agenda_audit or {}).get("selected_from_agenda")
            ),
            "agenda_type": (agenda_audit or {}).get(
                "selected_agenda_type", ""
            ),
            "purpose": (agenda_audit or {}).get(
                "selected_agenda_reason", ""
            ),
            "preparation_for": (agenda_audit or {}).get(
                "selected_preparation_for", ""
            ),
            "preparation_relation": (agenda_audit or {}).get(
                "selected_preparation_relation", ""
            ),
            "observation_evidence": (
                self._clean_text(observation)[:240] if match else ""
            ),
        }
    ...
    def _tried_here_context(
        self,
        location: str,
        current_world_fingerprint: dict,
        current_state_snapshot: dict | None = None,
    ) -> str:
        """Merge all attempt stores into one state-aware advisory block."""
        self._last_reopen_audit = []
        by_command: dict[str, dict] = {}
        ledger_records = self.attempt_ledger.counts_for_location(location)

        for command_key, record in ledger_records.items():
            key = normalize_command_key(
                record.get("command") or command_key
            )
            if not key:
                continue
            by_command[key] = {
                ...
                "outcome": self._clean_text(
                    record.get("last_outcome", "")
                )[:90],
                "attempts": int(record.get("count", 0) or 0),
                "last_step": int(record.get("last_step", 0) or 0),
                "outcomes": dict(record.get("outcomes", {}) or {}),
                "state_fingerprints": list(
                    record.get("state_fingerprints", []) or []
                ),
                "has_ledger": True,
            }

        for record in self.failed_action_memory.records_for_location(location):
            key = normalize_command_key(record.get("command", ""))
            if not key:
                continue
            entry = by_command.setdefault(key, {
                "command": self._clean_text(record.get("command", "")),
                "outcome": "",
                "attempts": 0,
                "last_step": 0,
                "outcomes": {},
            ...
            reason = self._clean_text(record.get("failure_reason", ""))
            if reason:
                entry["outcome"] = reason[:90]
            entry["attempts"] = max(
                int(entry.get("attempts", 0)),
                int(record.get("attempt_count", 1) or 1),
            )

        for record in self.state_action_memory.records_for_state(
            ...
            if not entry.get("outcome"):
                entry["outcome"] = self._clean_text(
                    record.get("reason")
                    or record.get("result_observation")
                    or ""
                )[:90]
        ...
        entries = sorted(
            by_command.values(),
            key=lambda item: (
                int(item.get("last_step", 0)),
                normalize_command_key(item.get("command", "")),
            ),
        )[-30:]
        lines = []
        for entry in entries:
            ...
            productive = any(
                int(outcomes.get(kind, 0) or 0) > 0
                for kind in ("state_change", "moved", "scored")
            )
            attempts = int(entry.get("attempts", 0) or 0)
            fingerprints = list(entry.get("state_fingerprints", []) or [])
            tier = "COVERED"
            reopened_reason = ""
            if (
                entry.get("has_ledger")
                and attempts >= 3
                and not productive
                and fingerprints
            ):
                current_key = self._snapshot_key(
                    current_world_fingerprint or {}
                )
                known_keys = {
                    self._snapshot_key(fingerprint)
                    for fingerprint in fingerprints
                    if fingerprint
                }
                if current_key and current_key not in known_keys:
                    reopened_reason = self._command_reopen_reason(
                        entry.get("command", ""),
                        current_world_fingerprint or {},
                        fingerprints,
                    )
                tier = "REOPENED" if reopened_reason else "EXHAUSTED"
            outcome = entry.get("outcome") or "outcome not recorded"
            outcome = outcome.replace('"', "'")
            line = (
                f"{entry.get('command', '')} [{tier}] "
                f"x{max(1, attempts)}, \"{outcome}\""
            )
            if reopened_reason:
                line += f" | {reopened_reason}"
            lines.append(line)
        lines.append(
    ...
    def _command_reopen_reason(
        self,
        command: str,
        current_fingerprint: dict,
        stored_fingerprints: list[dict],
    ) -> str:
        """Explain only retry-relevant world changes, not narrative changes."""
        current_inventory = set(current_fingerprint.get("inventory", []) or [])
        known_inventory = set()
        for fingerprint in stored_fingerprints:
            known_inventory.update(fingerprint.get("inventory", []) or [])
        gained_items = sorted(current_inventory - known_inventory)
        command_key = normalize_command_key(command)
        for item in gained_items:
            item_key = normalize_command_key(item)
            if item_key and re.search(
                rf"(?:^|\s){re.escape(item_key)}(?:$|\s)",
                command_key,
            ):
                self._last_reopen_audit.append({
                    "command": command,
                    "item": item,
                    "decision": "reopened",
                    "reason": "newly carried item is named in the command",
                })
                return f"you now carry the {item}, which this command names"
            memory = getattr(self, "situation_memory", None)
            active_situations = (
                memory.active_situations(
                    inventory=list(self.kg_map.inventory),
                    visible_objects=self._visible_objects_for_location(
                        self._effective_location()
                    ),
            ...
            for situation in active_situations:
                situation_location = normalize_location_key(
                    situation.get("location", "")
                )
                situation_text = normalize_command_key(
                    " ".join([
                        str(situation.get("situation", "")),
                        str(situation.get("possible_solution", "")),
                    ])
                )
                if (
                    situation_location
                    == normalize_location_key(self._effective_location())
                    and item_key
                    and re.search(
                        rf"(?:^|\s){re.escape(item_key)}(?:$|\s)",
                        situation_text,
                    ...
                    self._last_reopen_audit.append({
                        ...
                        "decision": "reopened",
                        "reason": (
                            "newly carried item is named by an active "
                            "same-location situation"
                        ),
                    })
                    return (
                        f"you now carry the {item}, which an active "
                        "situation here names"
                    )
            for goal in (
                memory.goal_situations() if memory is not None else []
            ):
                if goal.get("declined") or goal.get("status") == "confirmed":
                    continue
                gateway_matches = any(
                    normalize_location_key(gateway.get("room", ""))
                    == normalize_location_key(self._effective_location())
                    and normalize_command_key(gateway.get("command", ""))
                    == command_key
                    for gateway in goal.get("gateways", []) or []
                )
                if (
                    gateway_matches
                    and self._inventory_matches_requirements([item], goal)
                ):
                    self._last_reopen_audit.append({
                        "command": command,
                        "item": item,
                        "decision": "reopened",
                        "reason": (
                            "newly carried item matches an open goal reached "
                            "by this command"
                        ),
                    })
                    return (
                        f"you now carry the {item}, which matches the open "
                        "goal reached by this command"
                    )
            self._last_reopen_audit.append({
                "command": command,
                "item": item,
                "decision": "not_reopened",
                "reason": "newly carried item is unrelated to the command",
            })

        current_states = dict(
            current_fingerprint.get("object_states", {}) or {}
        )
        stored_states: dict[str, set[str]] = {}
        for fingerprint in stored_fingerprints:
            for name, state in (
                fingerprint.get("object_states", {}) or {}
            ).items():
                stored_states.setdefault(str(name), set()).add(str(state))

        named_objects = sorted(
            (
                name for name in current_states
                if name and name in command_key
            ),
            key=len,
            reverse=True,
        )
        for name in named_objects:
            current_state = str(current_states.get(name, ""))
            prior_states = stored_states.get(name, set())
            if prior_states and current_state not in prior_states:
                if current_state:
                    return (
                        f"the {name} is now {current_state}, unlike during "
                        "the earlier attempts"
                    )
                return f"the {name} state changed since the earlier attempts"
        return ""

    def _relevant_lessons_for_brainstorm(self) -> str:
        records = []
        allowed = {"achievement", "enabler", "death_warning"}
        for record in self._last_retrieved_records:
            metadata = record.get("metadata") or {}
            kind = str(metadata.get("kind") or metadata.get("trigger") or "")
            if kind in allowed:
        ...
        if not records:
            return "none"
        return self._format_experiences_for_prompt(
            records,
            candidate_pool=self._last_retrieved_records,
        )

    def _parse_setup_commands(self, summary: str,
                              scoring_action: str) -> list[str]:
        match = re.search(
            r"<setup_commands>\s*(.*?)\s*</setup_commands>",
            str(summary or ""),
            flags=re.IGNORECASE | re.DOTALL,
        )
        if not match:
            return []
        scoring_key = normalize_command_key(scoring_action)
        output = []
        seen = set()
        for value in match.group(1).split(";"):
            command = self._clean_text(value)
            key = normalize_command_key(command)
            if (
                not command
                or command.lower() == "none"
                or not key
                or key == scoring_key
                or key in seen
                or not self._is_causal_setup_command(command)
            ):
                continue
    ...
    def _is_causal_setup_command(self, command: str) -> bool:
        """Keep only plausible state-changing reward prerequisites."""
        command = self._clean_text(command).lower()
        if not command:
            return False
        if self._blocked_direction_for_command(command):
            return False
        if self._is_movement_action(command):
            return False
        first = command.split()[0]
        if first in {
            "enter", "exit", "leave", "climb", "descend", "ascend",
            "crawl", "cross", "follow", "jump",
        }:
            return False
        return first not in {
            "look", "examine", "read", "inspect", "peek", "check",
            "watch", "listen", "smell",
        }
    ...
    def _record_reward_directory_event(
        ...
        route_hops = compress_epoch_path(list(self._epoch_path))
        route_hint = render_route_hint(route_hops)
        entry = self.reward_directory.add_or_update({
            "event_key": event_key,
            "points": int(points or 0),
            "location": location,
            "location_registry_id": (
                self.kg_map.registry_id_for(location)
                if getattr(self, "kg_map", None) is not None
                else ""
            ),
            "scoring_command": scoring_command,
            "setup_commands": list(setup_commands or []),
            "route_hint": route_hint,
            "route_hops": [list(hop) for hop in route_hops],
            "first_seen_epoch": int(self.current_epoch),
        })
        if (
            event_key in getattr(
                self, "earned_score_event_keys_this_epoch", set()
            )
            and entry.get("event_key")
        ):
            self.earned_score_event_keys_this_epoch.add(entry["event_key"])
        return entry
    ...
    def _experience_lesson_signature(self, record: dict) -> str:
        ...
        try:
            ...
            if isinstance(parsed, dict):
                lesson = str(parsed.get("reusable_lesson", "") or "")
        except Exception:
            match = re.search(
                r'["\']?reusable_lesson["\']?\s*:\s*["\']([^"\']+)',
                text,
                flags=re.IGNORECASE,
            )
            lesson = match.group(1) if match else ""
        normalized = self._normalize_event_piece(lesson or text)
    ...
    def _retrieve_experiences_for_prompt(self, query: str) -> str:
        current_location = self.kg_map.current_location or ""
        current_registry_id = self.kg_map.registry_id_for(current_location)
        local_achievement_records = []
        local_enabler_records = []
        local_route_records = []
        if current_registry_id:
            local_achievement_records = (
                self.experience_lib.retrieve_relevant_structured(
                    query,
                    top_k=config.EXPERIENCE_FETCH_K,
                    fetch_k=config.EXPERIENCE_FETCH_K,
                    where={"$and": [
                        {"kind": "achievement"},
                        {"location_registry_id": current_registry_id},
                    ]},
                )
            )
            local_enabler_records = (
            ...
            local_route_records = (
                self.experience_lib.retrieve_relevant_structured(
                    ...
                    where={"$and": [
                        {"kind": "route"},
                        {"source_registry_id": current_registry_id},
                    ]},
                )
            )
        ...
        dropped_candidates = self._retrieval_drop_reasons(records, shown)
        self._last_retrieved_records = list(shown)
        self.last_retrieval_debug = {
            ...
            "local_filtered_fetch_counts": {
                "achievements": len(local_achievement_records),
                "enablers": len(local_enabler_records),
                "routes": len(local_route_records),
            },
    ...
    def _select_diverse_experiences(self, records: list[dict],
        ...
        # Retrieval supplies exact-room procedures. Cross-room reward pursuit
        # belongs to the reward directory and its route guidance.
    ...
    def _retrieval_drop_reasons(
        self,
        records: list[dict],
        shown: list[dict],
    ) -> list[dict]:
        """Explain why achievement/enabler candidates did not reach the prompt."""
        shown_ids = {
            self._experience_record_id(record) for record in (shown or [])
        }
        output = []
        for record in records or []:
            metadata = record.get("metadata") or {}
            kind = str(metadata.get("kind") or metadata.get("trigger") or "")
            if kind not in {"achievement", "enabler"}:
                continue
            rid = self._experience_record_id(record)
            if rid in shown_ids:
                continue
            if kind == "achievement" and self._achievement_earned_this_epoch(record):
                reason = "completed_this_epoch"
            elif kind == "enabler" and self._enabler_completed_this_epoch(record):
                reason = "completed_this_epoch"
            elif not self._experience_location_relevant(
                record,
                ...
                reason = "slot_taken"
            output.append({
                **self._experience_digest(record),
                "drop_reason": reason,
            })
        return output
    ...
    def _merge_experience_records(self, records: list[dict]) -> list[dict]:
        ...
        for record in records or []:
            ...
            lesson_key = (
                ...
                kind,
                lesson_signature,
            ) if location and lesson_signature else None
            if lesson_key in lesson_positions:
                index = lesson_positions[lesson_key]
                previous = merged[index]
                previous_metadata = previous.get("metadata") or {}
                previous_order = (
                    int(previous_metadata.get("epoch", 0) or 0),
                    int(previous_metadata.get("step", 0) or 0),
                )
                new_order = (
                    int(metadata.get("epoch", 0) or 0),
                    int(metadata.get("step", 0) or 0),
                )
            ...
            if lesson_key:
                lesson_positions[lesson_key] = len(merged) - 1
            seen.add(rid)
        return merged
    ...
    def _enabler_completed_this_epoch(self, record: dict) -> bool:
        ...
        linked_reward_earned = bool(
            key and key in self.earned_score_event_keys_this_epoch
        )
        if not linked_reward_earned:
            reward = metadata.get("enables_reward")
            location = (
                metadata.get("enables_location")
                or metadata.get("location")
            )
            linked_reward_earned = self._score_location_reward_earned(
                location,
                reward,
                location_fingerprint=metadata.get(
                    "enables_location_fingerprint",
                    "",
                ),
            )
        if (
            linked_reward_earned
            and self._enabler_still_needed_for_open_goal(record)
        ):
            return False
        return linked_reward_earned

    @staticmethod
    def _acquisition_target(command: str) -> str:
        normalized = normalize_command_key(command)
        match = re.fullmatch(
            r"(?:take|get|grab|acquire|collect|pick up)\s+(.+)",
            normalized,
        )
        return match.group(1).strip() if match else ""

    def _enabler_still_needed_for_open_goal(self, record: dict) -> bool:
        """Keep a learned acquisition visible while it prepares an open hazard."""
        metadata = record.get("metadata") or {}
        target = self._acquisition_target(metadata.get("enabler_action", ""))
        if not target:
            return False
        carried = {
            normalize_command_key(item)
            for item in self.kg_map.inventory
            if normalize_command_key(item)
        }
        if target in carried:
            return False
        for goal in self.situation_memory.goal_situations():
            if goal.get("declined") or goal.get("status") == "confirmed":
                continue
            if self._inventory_matches_requirements([target], goal):
                return True
        enabler_location = normalize_location_key(
            metadata.get("location", "")
        )
        if enabler_location:
            for situation in self.situation_memory.active_situations():
                if (
                    normalize_location_key(situation.get("location", ""))
                    != enabler_location
                ):
                    continue
                situation_text = normalize_command_key(
    ...
    def _experience_location_relevant(self, record: dict,
        ...
        metadata = record.get("metadata") or {}
        current = self._effective_location()
    ...
    def _experience_digest(self, record: dict) -> dict:
        ...
        return {
            ...
            "source_registry_id": metadata.get("source_registry_id", ""),
            "location_fingerprint": metadata.get("location_fingerprint", ""),
    ...
    def _format_experiences_for_prompt(self, records: list[dict],
        ...
        for i, record in enumerate(records, 1):
            ...
            if kind == "achievement":
                ...
                still_needed = self._enabler_still_needed_for_open_goal(record)
                if self._enabler_completed_this_epoch(record):
                    ...
                    if still_needed:
                        header_bits.append("still_needed_for_open_goal=true")
                        header_bits.append("use_as=prepare_for_open_hazard")
                    else:
    ...
    def _room_danger_history_for_prompt(self, location: str) -> str:
        """Render accumulated death evidence for the current room.

        This is an explicit view over the existing persistent hazard evidence,
        not a second death-memory store. Exact room identity is preferred; a
        base-title fallback keeps older evidence useful when a registry alias
        changed between epochs.
        """
        clean_location = self._clean_text(location)
        if not clean_location:
            return "none recorded for current room"

        target_key = self._hazard_room_identity_key(
            clean_location,
            self._room_memory_identity(clean_location),
        )
        target_base = target_key.split("|", 1)[0]
        evidence_by_room = getattr(self, "_hazard_room_evidence", {}) or {}
        deaths_by_room = getattr(self, "_hazard_room_deaths", {}) or {}

        matching_keys = [target_key] if (
            target_key in evidence_by_room or target_key in deaths_by_room
        ) else [
            key for key in set(evidence_by_room) | set(deaths_by_room)
            if str(key).split("|", 1)[0] == target_base
        ]
        if not matching_keys:
            return "none recorded for current room"

        death_count = sum(
            int(deaths_by_room.get(key, 0) or 0)
            for key in matching_keys
        )
        actions: list[str] = []
        seen_actions: set[str] = set()
        for key in matching_keys:
            for evidence in evidence_by_room.get(key, []) or []:
                action = self._clean_text(evidence.get("fatal_action", ""))
                action_key = normalize_command_key(action)
                if action and action_key and action_key not in seen_actions:
                    actions.append(action)
                    seen_actions.add(action_key)

        lines = [f"Deaths here: {max(death_count, 1)}"]
        lines.append("Observed fatal commands:")
        if actions:
            lines.extend(f"- {action}" for action in actions[:6])
        else:
            lines.append("- exact command unavailable")
        lines.append(
            "This room has an observed lethal hazard. An unrecorded command "
    ...
    def _route_for_current_room(self, record: dict) -> bool:
        ...
        source = (
            ...
            or metadata.get("location_issued")
            or metadata.get("prev_location")
            or ""
        )
        if not current or not source:
            return False
        source_registry_id = str(metadata.get("source_registry_id") or "")
        current_registry_id = self.kg_map.registry_id_for(current)
        if source_registry_id and current_registry_id:
            return source_registry_id == current_registry_id
        return normalize_location_key(source) == normalize_location_key(current)
    ...
    def _world_fingerprint(
        ...
        location: str,
        inventory: list,
        visible_objects_with_state: list,
    ) -> dict:
        """Stable pre-command world state used for context-sensitive retries."""
        object_states = {}
        for item in visible_objects_with_state or []:
            if isinstance(item, dict):
                name = normalize_location_key(item.get("name", ""))
                state = normalize_location_key(item.get("state", ""))
            else:
                name = normalize_location_key(item)
                state = ""
            if name:
                object_states[name] = state
        return {
            "location": normalize_location_key(location) or "unknown",
            "inventory": sorted({
                normalize_location_key(item)
                for item in (inventory or [])
                if normalize_location_key(item)
            }),
            "object_states": dict(sorted(object_states.items())),
        }
    ...
    def _detect_and_store_situation(self, action: str, observation: str,
        ...
        try:
            response_body = self.llm.detect_stored_situation(
                ...
                observation=observation,
                inventory=result["inventory"],
                stored_situations=persistent_before,
                location_uncertain=bool(self.kg_map.location_uncertain),
                entered_via=(
                    self.kg_map.location_entered_dark_via
                    if self.kg_map.location_uncertain else ""
                ),
            )
            ...
            if parse_error:
                ...
                if self.kg_map.location_uncertain:
                    parsed = dict(parsed)
                    last_known = self.kg_map.current_location or "unknown"
                    entered_via = (
                        self.kg_map.location_entered_dark_via or "unknown action"
                    )
                    parsed["location"] = last_known
                    suffix = (
                        f" (encountered after '{entered_via}' from "
                        f"{last_known}; destination not yet identified)"
                    )
                    situation_text = self._clean_text(
                        parsed.get("situation", "")
                    )
                    if suffix.lower() not in situation_text.lower():
                        parsed["situation"] = f"{situation_text}{suffix}".strip()
                    result["uncertain_location_grounding"] = {
                        "stored_location": parsed["location"],
                        "last_confirmed_location": last_known,
                        "entered_via": entered_via,
                        "synthetic_location_persisted": False,
                    }
                ...
                if self.situation_memory.key_for(parsed) in just_resolved_keys:
                    ...
                    stored, stored_situation = self.situation_memory.add(
                        parsed,
                        created_epoch=int(getattr(self, "current_epoch", 0) or 0),
                        created_step=int(getattr(self, "step_count", 0) or 0),
                    )
                    if stored:
        ...
        effective_location = self._effective_location()
        effective_visible_objects = self._visible_objects_for_location(
            effective_location
        )
        result["active_situations_after"] = self.situation_memory.active_situations(
            inventory=list(self.kg_map.inventory),
            visible_objects=effective_visible_objects,
        )
        return result
    ...
    def _resolve_stored_situations(self, action: str, observation: str,
        ...
        if not worth_checking:
            ...
            result["active_situations_after"] = (
                self.situation_memory.active_situations(
                    inventory=list(self.kg_map.inventory),
                    visible_objects=visible_objects,
                )
    ...
    def _ground_death_room_from_summary(self, summary: str,
        ...
        if not raw:
            return "", {"status": "empty"}
        terminal_text = str(terminal_observation or "")
        if raw not in terminal_text:
            return "", {"status": "ungrounded", "raw_title": raw}
        death_marker = re.search(r"\*\s*\*\s*\*", terminal_text)
        if (death_marker
                and terminal_text.find(raw) >= death_marker.start()):
            return "", {"status": "after_death_marker", "raw_title": raw}
    ...
    def _goal_hazard_context(self, action: str, action_valid,
                             ...
                             location_after: str,
                             grounded_death_room: str = "",
                             was_location_uncertain: bool = False,
                             uncertain_entered_via: str = "",
                             uncertain_last_known: str = "") -> dict:
        ...
        if was_location_uncertain and uncertain_entered_via:
            gateway_room = self._first_known_location(
                uncertain_last_known, issued, "Starting Location"
            )
            return {
                "hazard_location": gateway_room,
                "hazard_fingerprint": (
                    self.kg_map.registry_id_for(gateway_room)
                    or self._location_fingerprint_hash(gateway_room)
                ),
                "hazard_registry_id": self.kg_map.registry_id_for(gateway_room),
                "gateway": {
                    "room": gateway_room,
                    "fingerprint": (
                        self.kg_map.registry_id_for(gateway_room)
                    ...
                    "command": self._clean_text(uncertain_entered_via),
                },
                "source": "unseen_area_entry_gateway",
            }
        destination = ""
        hazard_registry_id = ""
        source = ""
    ...
    def _handle_death_goal_lifecycle(self, event_key: str,
                                     ...
                                     gateway: dict = None,
                                     death_summary: str = "") -> tuple[dict, dict]:
        ...
        death_summary = death_summary or self._death_summary_for_event(event_key)
        fatal_assessment = self._death_summary_assessment(death_summary)
        transition["fatal_action_assessment"] = fatal_assessment
        ...
        if existing:
            ...
            if (room_death_count < 2
                    and fatal_assessment != "wrong_in_that_state"):
        ...
        if (not existing
                and room_death_count == 1
                and fatal_assessment == "wrong_in_that_state"
                and (not parsed.get("preparable") or not parsed.get("requires"))):
            transition["status"] = "first_death_no_concrete_requirements"
            return transition, log_entry
    ...
    def _death_summary_assessment(self, summary: str) -> str:
        """Read the grounded assessment emitted inside a death summary."""
        ...
        try:
            ...
            return ""
        assessment = self._clean_text(
            parsed.get("fatal_action_assessment", "")
        ).lower()
        if assessment in {
            "wrong_in_that_state", "wrong_in_itself", "unlucky_randomness"
        }:
            return assessment
        return ""
    ...
    def _store_reward_enabler_experiences(self, score_event_key: str,
                                          ...
                                          location_after: str,
                                          setup_commands: list[str] = None) -> list[dict]:
        ...
        for setup_command in setup_commands or []:
            setup_key = normalize_command_key(setup_command)
            if (
                not setup_key
                or setup_key == scoring_key
                or setup_key in seen_commands
                or not self._is_causal_setup_command(setup_command)
            ):
                continue
            matching = next(
                (
                    item for item in reversed(self._recent_outcomes)
            ...
            candidates.append(
                dict(matching) if matching else {
                    "epoch": int(self.current_epoch),
                    "step": int(self.step_count),
                    "location": scoring_location,
                    "destination": scoring_location,
                    "command": setup_command,
                    "observation": "",
                    "outcome_class": "summary_setup",
                    "reward_change": 0,
                }
            )
            seen_commands.add(setup_key)

        old_path_candidates = []
        for item in self._recent_outcomes[-5:]:
            enabler_key = normalize_command_key(item.get("command", ""))
            if (
                item.get("outcome_class") not in {"state_change", "info"}
                or not enabler_key
                or enabler_key == scoring_key
                or enabler_key in seen_commands
                or not self._is_causal_setup_command(
                    item.get("command", "")
                )
            ):
                continue
            old_path_candidates.append(dict(item))
            seen_commands.add(enabler_key)
        remaining_slots = max(0, 3 - len(candidates))
        if remaining_slots:
            candidates.extend(old_path_candidates[-remaining_slots:])
    ...
    @staticmethod
    def _direction_state_key(location: str, direction: str) -> tuple[str, str]:
        return (
            normalize_location_key(location) or "unknown",
            normalize_command_key(direction),
        )

    def _same_room_state_changed_after(
        self,
        location: str,
        evidence_step: int,
    ) -> bool:
        if not int(evidence_step or 0):
            return False
        state_steps = getattr(self, "_room_state_change_steps", {})
        return int(
            state_steps.get(normalize_location_key(location), 0) or 0
        ) > int(evidence_step)
    ...
    def _empty_navigation_guard_debug(self) -> dict:
        ...
        return {
            ...
            "revision_final_command": "",
            "revision_insisted": False,
            "direction": "",
            "failed_this_visit": 0,
            "last_message": "",
            ...
            "final_command": "",
            "grounding_scope": "",
            "known_movement_destination": "",
        }

    def _known_movement_destination(self, command: str) -> str:
        """Return a destination proven by the KG or this epoch's attempt ledger."""
        if self.kg_map.location_uncertain:
            return ""
        command_key = normalize_command_key(command)
        if not command_key:
            return ""
        current = self.kg_map.current_location or ""
        node = self.kg_map.nodes.get(current, {})
        direction = self._blocked_direction_for_command(command)
        if direction:
            destination = str(
                (node.get("direction", {}) or {}).get(direction, "")
            )
            if destination:
                return destination
        for transition_command, transition_destination in (
            node.get("confirmed_actions", {}) or {}
        ).items():
            if normalize_command_key(transition_command) == command_key:
                destination = str(transition_destination or "")
                if destination:
                    return destination
        ...
        if ledger is None:
            return ""
        record = ledger.counts_for_location(current).get(command_key, {})
        if int((record.get("outcomes", {}) or {}).get("moved", 0)) <= 0:
            return ""
    ...
    def _selected_command_grounding_violation(
        self,
        command: str,
        tried_here: str,
    ) -> str:
        """Return factual evidence that warrants one LLM reconsideration."""
        command_key = normalize_command_key(command)
        if not command_key:
            return ""
        grounded_rider = self._selected_command_grounded_rider(command)
        known_movement_destination = self._known_movement_destination(command)
        selected_command_exhausted = False

        for line in str(tried_here or "").splitlines():
            match = re.match(r"^(.*?)\s+\[EXHAUSTED\](?:\s|$)", line.strip())
            if (
                match
                and normalize_command_key(match.group(1)) == command_key
                and not grounded_rider
                and not known_movement_destination
            ):
                selected_command_exhausted = True
                break

        if not self.kg_map.location_uncertain:
            current = self.kg_map.current_location or ""
            memory = getattr(self, "situation_memory", None)
            goals = memory.goal_situations() if memory is not None else []
            for goal in goals:
                if goal.get("declined") or goal.get("status") == "confirmed":
                    continue
                for gateway in goal.get("gateways", []) or []:
                    if (
                        normalize_location_key(gateway.get("room", ""))
                        != normalize_location_key(current)
                        or normalize_command_key(gateway.get("command", ""))
                        != command_key
                    ):
                        continue
                    if not self._inventory_matches_requirements(
                        list(self.kg_map.inventory),
                        goal,
                    ):
                        requirements = (
                            goal.get("requires")
                            or goal.get("item_keywords")
                            or ["recorded preparation"]
                        )
                        return (
                            f"'{command}' enters the open hazard at "
                            f"'{goal.get('hazard_location', 'unknown')}', but "
                            "the current inventory does not match its recorded "
                            f"preparation: {', '.join(requirements)}"
                        )
        if selected_command_exhausted:
            return (
                f"'{command}' is marked EXHAUSTED in the current "
                "location under the current relevant world conditions"
            )
        return ""

    def _selected_command_grounded_rider(self, command: str) -> str:
        """Return a route, reward, or prepared-goal reason for this command."""
        if self.kg_map.location_uncertain:
            return ""
        command_key = normalize_command_key(command)
        current = self.kg_map.current_location or ""
        ...
        if not destination:
            for transition_command, transition_destination in (
                ...
                if normalize_command_key(transition_command) == command_key:
                    ...
                    break
        if destination:
            rider = self._room_familiarity_by_location().get(
                destination,
                {},
            ).get("rider", "")
            if rider:
        ...
        directory = getattr(self, "reward_directory", None)
        if directory is not None:
            earned = set(
                getattr(
                    self,
                    "earned_score_event_keys_this_epoch",
                    set(),
                )
            )
            for entry in directory.unearned_entries(earned):
                if (
                    ...
                    continue
                candidate_commands = [
                    entry.get("scoring_command", ""),
                    *(entry.get("setup_commands", []) or []),
                ]
                if any(
                    normalize_command_key(candidate) == command_key
                    for candidate in candidate_commands
                ):
                    return "exact command for a known unearned reward here"

        memory = getattr(self, "situation_memory", None)
        for goal in memory.goal_situations() if memory is not None else []:
            if goal.get("declined") or goal.get("status") == "confirmed":
                continue
            gateway_matches = any(
                normalize_location_key(gateway.get("room", ""))
            ...
            if gateway_matches and self._inventory_matches_requirements(
                list(self.kg_map.inventory),
                goal,
            ):
                return "prepared gateway for an active goal"
        return ""
    ...
    def _apply_navigation_enforcement(self, command: str, raw_llm_response: str,
                                      ...
                                      observation: str,
                                      affordance_agenda: list,
                                      tried_here: str = "") -> tuple:
        """Rate-limit repeated rejected exits without permanently prohibiting them."""
        debug = self._empty_navigation_guard_debug()
        debug["initial_command"] = command
        debug["final_command"] = command
        known_movement_destination = self._known_movement_destination(command)
        if known_movement_destination:
            debug["grounding_scope"] = "movement_advisory_only"
            debug["known_movement_destination"] = known_movement_destination
        grounding_violation = self._selected_command_grounding_violation(
            command,
            tried_here,
        )
        if grounding_violation:
            ...
            debug["layer"] = 1
            debug["revision_trigger"] = "grounding_consistency"
            debug["revision_reason"] = grounding_violation
            revision_prompt = (
                f"{prompt}\n\n"
                "GROUNDING CONSISTENCY CHECK:\n"
                f"You selected '{command}', but current memory says: "
                f"{grounding_violation}.\n"
                "Reconsider once. Choose a different executable command unless "
                "you can cite exact current evidence: a specifically relevant "
                "new item, an explicit target-state change, a route waypoint, "
                "an unearned reward, or an active goal. Vague claims that the "
                "world may have changed, that a new perspective may help, or "
                "that inventory is merely different are not evidence. You may "
                "return the same command only if you name such exact evidence."
            )
            revision_raw = self.llm.chat(
                system_prompt=(
                    "You are an expert player of text-based interactive "
                    "fiction games."
                ),
                user_prompt=revision_prompt,
                think=True,
            )
            revised_command = self._parse_command(revision_raw)
            debug["revision_raw_response"] = revision_raw
            debug["revision_final_command"] = revised_command
            debug["revision_insisted"] = (
    ...
    def _prompt_kg_map_context(self) -> str:
        ...
        familiarity = self._room_familiarity_by_location()
        context = self.kg_map.to_clean_dict(
            active_goal_locations=self._active_goal_locations(),
            familiarity_by_location=familiarity,
        )
    ...
    def _record_initial_room_familiarity(self):
        location = self.kg_map.current_location or "unknown"
        registry_id = self.kg_map.registry_id_for(location)
        if not registry_id:
            return
        self.room_familiarity.visit(
            registry_id,
            location,
            self.current_epoch,
        )
        self.room_familiarity.observe_objects(
            registry_id,
            location,
            self._visible_objects_for_location(location),
        )
        self.room_familiarity.update_described_exits(
            registry_id,
            location,
            self._described_exits_untried(location),
        )

    def _update_room_familiarity(
        self,
        location_changed: bool,
        source_location: str,
        reward_change: int,
        outcome_class: str,
        situation_detail: dict,
        kg_location_resolution: dict,
    ) -> dict:
        labels_before = self._room_familiarity_by_location()
        current = self.kg_map.current_location or "unknown"
        current_id = self.kg_map.registry_id_for(current)
        if location_changed and current_id:
            self.room_familiarity.visit(
                current_id,
                current,
                self.current_epoch,
            )
        new_objects = self.room_familiarity.observe_objects(
            current_id,
            current,
            self._visible_objects_for_location(current),
        )
        if current_id:
            self.room_familiarity.update_described_exits(
                current_id,
                current,
                self._described_exits_untried(current),
            )

        novelty = []
        source_id = self.kg_map.registry_id_for(source_location)
        if reward_change > 0 and source_id:
            self.room_familiarity.record_novelty(
                source_id, source_location, "reward"
            )
            novelty.append({"location": source_location, "kind": "reward"})
        if outcome_class == "state_change" and source_id:
            self.room_familiarity.record_novelty(
                source_id, source_location, "state_change"
            )
            novelty.append({
                "location": source_location,
                "kind": "state_change",
            })
        transition = kg_location_resolution.get("confirmed_transition", {})
        if (
            transition
            and transition.get("from")
            and kg_location_resolution.get("newly_confirmed_exit")
        ):
            transition_source = transition.get("from")
            transition_source_id = self.kg_map.registry_id_for(
                transition_source
            )
            if transition_source_id:
                ...
                novelty.append({
                    "location": transition_source,
                    "kind": "new_confirmed_exit",
                })
        stored = situation_detail.get("new_stored_situation")
        if stored:
            situation_location = stored.get("location") or current
            situation_id = self.kg_map.registry_id_for(situation_location)
            if situation_id:
                self.room_familiarity.record_novelty(
                    situation_id,
                    situation_location,
        ...
        if new_objects:
            novelty.append({
                "location": current,
                "kind": "new_visible_object",
                "objects": new_objects,
            })
        labels_after = self._room_familiarity_by_location()
        tier_transitions = []
        for location in sorted(set(labels_before) | set(labels_after)):
            before = labels_before.get(location, {})
            after = labels_after.get(location, {})
            ...
            tier_transitions.append({
                ...
                "to": after.get("tier", ""),
                "visits_epoch": after.get("visits_epoch", 0),
                "last_novelty_visit_index": after.get(
                    "last_novelty_visit_index",
                    0,
                ),
                "visits_since_novelty": after.get(
                    "visits_since_novelty",
                    0,
                ),
            })
        return {
            "current_location": current,
            "registry_room_id": current_id,
            "novelty_events": novelty,
            "tier_transitions": tier_transitions,
            "rooms": labels_after,
        }

    def _described_exits_untried(self, location: str) -> list[str]:
        node = self.kg_map.nodes.get(location or "", {})
        return [
            direction
            for direction in node.get("described_directions", [])
            if (
                direction not in node.get("direction", {})
                and direction not in node.get("blocked_directions", [])
            )
        ]

    def _room_familiarity_by_location(self) -> dict[str, dict]:
        output = {}
        familiarity_store = getattr(
            self,
            "room_familiarity",
            RoomFamiliarity(),
        )
        earned = set(
            getattr(self, "earned_score_event_keys_this_epoch", set())
        )
        active_goal_keys = {
            normalize_location_key(value)
            for value in (
                self._active_goal_locations()
                if hasattr(self, "situation_memory")
                else []
            )
        }
        unearned = (
        ...
        for location in self.kg_map.nodes:
            ...
            if registry_id:
                familiarity_store.update_described_exits(
                    registry_id,
                    location,
                    described,
                )
            tier = familiarity_store.tier(registry_id, described)
            rider = ""
            location_key = normalize_location_key(location)
            reward_entry = next(
                (
                    entry for entry in unearned
                    if normalize_location_key(entry.get("location", ""))
                    == location_key
                ),
                None,
            )
            waypoint_entry = next(
                (
                    entry for entry in unearned
                    if any(
                        normalize_location_key(hop[0]) == location_key
                        for hop in entry.get("route_hops", [])
            ...
            if reward_entry:
                rider = (
                    f"known unearned +{reward_entry.get('points', 0)} "
                    "reward room"
                )
            elif waypoint_entry:
                rider = (
                    "waypoint on the route to "
                    f"+{waypoint_entry.get('points', 0)} at "
                    f"{waypoint_entry.get('location', 'unknown')}"
                )
            elif location_key in active_goal_keys:
                rider = "ACTIVE GOAL there"
            if rider and tier.get("tier") == "EXHAUSTED":
                tier["tier"] = "COVERED"
            if rider:
                tier["rider"] = rider
    ...
    def _active_goal_locations(self) -> list[str]:
        locations = []
        memory = getattr(self, "situation_memory", None)
        situations = memory.goal_situations() if memory is not None else []
        for situation in situations:
            for field in ("location", "hazard_location"):
                value = self._clean_text(situation.get(field, ""))
                if value and value not in locations:
                    locations.append(value)
            gateways = list(situation.get("gateways", []) or [])
            if situation.get("gateway"):
                gateways.append(situation.get("gateway"))
            for gateway in gateways:
                gateway_room = self._clean_text(
                    (gateway or {}).get("room", "")
                )
                if gateway_room and gateway_room not in locations:
                    locations.append(gateway_room)
        return locations

    def _visit_advisory_context(self, active_situations: list = None) -> dict:
        if self.kg_map.location_uncertain:
            return {}
        goal_locations = []
        memory = getattr(self, "situation_memory", None)
        for situation in (
            ...
            for gateway in gateways:
                ...
                if gateway_room:
                    goal_locations.append(gateway_room)
        state = self.kg_map.to_clean_dict(
            active_goal_locations=goal_locations,
            familiarity_by_location=self._room_familiarity_by_location(),
        ).get("current_room_state", {})
        return {
            "current_room": self.kg_map.current_location,
            "current_room_visit": state.get("visit_advisory", ""),
            "exits": state.get("exit_visit_advisory", {}),
            "action_transitions": state.get(
                "action_transition_visit_advisory", {}
            ),
        }

    def _recent_path_for_prompt(self) -> str:
        if not self._recent_outcomes:
            return "No completed movement path yet."
        parts = []
        for outcome in self._recent_outcomes[-8:]:
            source = self._clean_text(outcome.get("location", "")) or "unknown"
            destination = self._clean_text(
                outcome.get("destination", "")
            ) or source
            command = self._clean_text(outcome.get("command", "")) or "unknown"
            source_visits = int(self.kg_map.visit_counts.get(source, 0))
            destination_visits = int(
                self.kg_map.visit_counts.get(destination, 0)
            )
            parts.append(
                f"{source} (visited {source_visits}x): {command} -> "
                f"{destination} (visited {destination_visits}x)"
            )
        return " | ".join(parts)
    ...
    def _format_death_history(self, uncertainty_since_step=None) -> str:
        """Keep final turns plus repeated unseen-area evidence for a death."""
        exchanges = list(self.history)
        if not exchanges:
            return "No history yet. This is the start of the game."
        final_start = max(0, len(exchanges) - 4)
        selected_indices = set(range(final_start, len(exchanges)))
        if uncertainty_since_step is not None:
            steps_since = max(
                1,
                int(self.step_count) - int(uncertainty_since_step) + 1,
            )
            uncertainty_start = max(0, len(exchanges) - steps_since)
            counts = {}
            for _, obs in exchanges[uncertainty_start:]:
                exact = str(obs or "").strip()
                counts[exact] = int(counts.get(exact, 0)) + 1
            repeated_earlier = [
                index
                for index in range(uncertainty_start, final_start)
                if counts.get(str(exchanges[index][1] or "").strip(), 0) >= 3
            ][-6:]
            selected_indices.update(repeated_earlier)
        selected = [
            exchange for index, exchange in enumerate(exchanges)
            if index in selected_indices
        ][-10:]
        return self._format_exchange_list(selected)

    def _death_uncertainty_context(self, was_uncertain: bool,
                                   entered_via: str,
                                   last_known_location: str,
                                   uncertainty_since_step) -> str:
        if not was_uncertain:
            return "Context: no additional grounded context."
        steps_before = max(
            0,
            int(self.step_count) - int(
                uncertainty_since_step
                if uncertainty_since_step is not None else self.step_count
            ),
        )
        return (
            "Context: before dying, the player entered an unseen dark area via "
            f"'{entered_via or 'unknown action'}' from "
            f"{last_known_location or 'unknown'}, {steps_before} steps before "
            "this death, and could not see afterwards."
        )

    def _format_exchange_list(self, exchanges: list[tuple[str, str]]) -> str:
        output = []
        for index, (action, obs) in enumerate(exchanges, start=1):
            output.append(f"Turn {index}:")
            output.append(f"  Action: {action}")
            obs_short = obs[:300] + "..." if len(obs) > 300 else obs
            output.append(f"  Observation: {obs_short}")
        return "\n".join(output)

    def _format_history(self, limit: int = None) -> str:
        """Format the recent history for prompt inclusion."""
        if not self.history:
            return "No history yet. This is the start of the game."

        exchanges = self.history[-limit:] if limit else self.history
        return self._format_exchange_list(exchanges)
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "reward_directory_entries": len(self.reward_directory),
            "interaction_stat_records": len(self.interaction_stats),
            "situations_stored": len(self.situation_memory.active_situations()),
            "persistent_situations": len(
                self.situation_memory.persistent_situations()
            ),
            "situations_resolved_this_epoch": len(
                self.situation_memory.resolved_this_epoch()
            ),
