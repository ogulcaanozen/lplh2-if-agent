...
- In <setup_commands>, list only commands copied exactly from the supplied recent action and observation pairs that changed something the scoring command depended on, such as opening, moving, unlocking, activating, or taking a required object. Movement between rooms and pure observation commands such as look, examine, read, peek, and inspect are not setup commands. List causal setup in execution order separated by semicolons. Example: before "climb through window", "open window" is valid setup; "east" and "look at window" are not. If none, output <setup_commands>none</setup_commands>.
...
- After the existing tags, output exactly one line: <setup_commands>command1; command2</setup_commands>, or <setup_commands>none</setup_commands>.
...
1. FINAL EXCHANGES: copy the supplied action/observation pairs verbatim and in order, ending with the fatal one. The supplied history contains the final turns and may also contain earlier exactly repeated observations that ground a persistent unsafe condition. Copy all supplied pairs, up to 10.
2. PROXIMATE CAUSE: one sentence naming what state or condition directly led to the loss, supported only by the quoted text. Write it from the room where the fatal command was issued ({location_issued}). When the fatal command was movement, name the destination explicitly: "From <issuing room>, going <direction> leads to <destination> where you died". Never phrase the rule as if the fatal command was issued in the destination room.
3. CONFIRMED MECHANICS: list game rules or hazards the observations stated outright. Each confirmed mechanic must be traceable to a specific quoted observation or the attempt ledger; if it requires inference about causes, move it to untested_idea. If the attempt ledger says the fatal command produced different outcomes across attempts, include that outcomes vary between attempts. For fatal movement, state the mechanic from {location_issued} and name the destination where death occurred. Never attribute the fatal direction to the destination room. Do not invent weaknesses, tools, routes, or solutions that were never observed.
...
- New Rooms First Sweep: when the current room is labeled FRESH and has untried described exits, trying those exits early usually reveals more than exhausting every object here.
...
- **Inventory Is Authoritative**: The Inventory list shows everything you carry. Never assume you possess an object that is not listed there. Before you light, use, wear, give, read, or rely on an object, confirm it appears in Inventory. Visible, open, accessible, or switched on in the room is not the same as carried. If a visible object is needed for a known hazard or stored situation but is not listed in Inventory, acquire it before leaving for that hazard. Opening a container never moves its contents into Inventory.
- **Purposeful Acquisition**: Do not take an object merely because it is visible, portable, fresh, or untried. Prefer acquisition when the object has a concrete grounded purpose: it addresses a stored situation, death lesson, known reward procedure, current obstacle, or has a clear reusable function across plausible future interactions. A small amount of selective object exploration is reasonable while carrying capacity is unconstrained, but unsupported collecting must not crowd out known-purpose tools.
- **Capacity Pressure**: When the game explicitly reports that carrying capacity prevents acquiring a needed resource, compare the grounded purposes of carried items. Prefer keeping items tied to unresolved situations, hazards, rewards, or broadly reusable functions, and consider dropping an item whose known purpose is completed or unsupported before abandoning the needed resource. Do not invent usefulness for an item just because it is already carried.
- **Stored Situations**: Review unresolved hazards/blockers learned in this or an earlier epoch. Because the game world resets between epochs, a previously solved situation may be unresolved again when it reappears. If your current location, inventory, or known map makes one actionable now, consider addressing it; otherwise continue useful exploration.
...
- **Grounded Agenda Choice**: Treat agenda order as evidence while retaining final control. Prefer pending commands that target a confirmed visible or carried object, have not been tried in the current relevant state, and address unfinished information or a possible state change. Within an applicable entry, consider pending commands in their displayed order. If one command produced information but no lasting progress, prefer a different grounded untried command from that entry before inventing commands for descriptive words that are not established objects. Do not bypass a higher-ranked grounded command merely because another command sounds plausible; bypass it only for concrete evidence from the observation, inventory, route, reward memory, or stored situation.
...
- **Preparation Agenda**: PREPARATION is reserved for a validated, grounded resource that must be acquired, carried, equipped, or activated for a separate situation or hazard. Direct attempts on the obstacle itself are ordinary interactions. If a validated PREPARATION item applies here, establishing that resource takes priority over leaving for the hazard.
- **Tried Here**: one combined list shows every command already attempted at this location with its outcome and attempt count. COVERED means it has been tried but is not established as exhausted. EXHAUSTED means it has repeatedly produced no movement, score, or lasting state change under the same relevant world conditions; choose something else. REOPENED means an item named by that command or the named object's state changed since those attempts, so retrying may be reasonable but is not mandatory. An unrelated new inventory item, narrative wording, a new perspective, or a score change does not reopen a command. Any command not listed has never been tried here and is your best default candidate. When nothing else decides it, prefer in this order: unlisted, then REOPENED, then COVERED, then EXHAUSTED last. Never use an EXHAUSTED command as a setup step for another command.
- **Object History Notes**: some visible objects carry a note summarizing many failed attempts across earlier epochs with no score and no lasting change. Treat such an object as exhausted for now. Do not spend more commands on it unless you carry a new item or the observation explicitly shows it changed. This is advice, not a ban.
...
- **Reward Enablers**: If an enabler header says linked_reward_not_earned_this_epoch=true and exact_enabler_action happened in this exact room, treat that exact enabler action as important setup before trying the linked scoring action. A header with still_needed_for_open_goal=true remains relevant as preparation even when its original linked reward was already earned. If linked_reward_earned_this_epoch=true and no open goal still needs it, do not repeat the setup merely for reward.
...
- **Death Warnings Outrank Familiarity Labels**: If a retrieved death_warning names this room and its exact_fatal_action or also_fatal_here matches the command you are considering, do not issue that command. A FRESH label, an unexplored feeling, or a known unearned reward in this room is not a reason to retest a command that has already killed you. Familiarity labels describe how much you have explored, not whether an action is safe. When room_death_count is above 1, treat that command as settled and choose a different exit or action. If every recorded exit here is fatal, retreat the way you came.
- **Room Danger History**: When this section reports prior deaths in the current room, treat the room as containing an active lethal hazard even if your proposed command is not one of the recorded fatal commands. An unrecorded command is not evidence of safety. Exact fatal commands remain the strongest warning, but before ordinary exploration prefer grounded preparation, defense, a meaningfully different combat tactic, or retreat. This is hazard evidence, not a blanket ban on every command in the room.
...
- **Known Scoring Opportunities**: This section lists score events from earlier epochs with the room, the exact scoring command, the same room setup commands, and a route hint. Entries marked NOT EARNED this epoch are the highest value known targets right now. When no local action clearly advances the game, moving along the route hint toward the nearest unearned entry is the best default plan. Entries marked already earned this epoch give no points again, do not repeat them for reward. A single failed route hop does not disprove the route: re-approach from a neighboring room, and consider that a room name may appear in more than one place.
- **Missing Reward Resources**: Before leaving the current room, compare every NOT EARNED reward's exact setup commands with Inventory and visible objects here. If a setup command names a visible object that is not carried, treat it as a potentially important missing setup resource and consider acquiring it before continuing toward the reward or taking a hazardous transition. Ground this link in the recorded setup command itself; do not invent a resource requirement from object names alone. This is strong advice, not a forced action.
- **Nearby Side Rewards**: A side reward one or two hops off your current route, which returns you to the route afterwards, is usually worth taking while you still have step budget in this epoch.
- **Prefer Proven Progress**: when retrieved experiences name different exact commands on the same object, prefer the one whose action produced a lasting change or a score. An observation that reports a result and then reverts it in the same message ("once again concealed", "swings shut", "slips back") is not progress, and the command that produced it is not the one to repeat.
- **Two-command loops**: if your recent commands alternate between two commands in this room with unchanged results, that is a loop even when each one looks like a step of a plan. Do not issue either again.
- **Grounded Retry Justification**: Before choosing a COVERED or EXHAUSTED command, identify exact current evidence that changes its likely outcome. Claims such as "the world may have changed", "a new perspective may help", "the inventory is different", or "it is a safe transition" are not evidence. Name the specific changed object, the newly carried item used by this command, the explicit route waypoint, or the active goal. If you cannot, choose another command.
- **Exhausted Destinations**: Before movement, inspect the destination familiarity in the map's exit/action-transition advisory. Do not return to an EXHAUSTED destination merely because the movement command is a common direction or traversal action. Ordinary directions remain reusable as route steps in other rooms; only this location-to-destination choice is exhausted. Return only for a concrete route waypoint, unearned reward, active goal, or explicit new evidence tied to that destination.
...
- Avoid exact commands from the Tried Here list unless the current state has meaningfully changed and you can justify retrying.
...
- VISITED-PLACES ADVISORY: before choosing a movement command, check where it leads and how many times you have already visited that room this epoch. If you have already been there several times, assume you have seen what it currently has to offer; another visit is low value, so prefer an unexplored exit or a room with an active goal instead. This is advice, not a ban. Returning to a much-visited room is justified when you can state a concrete reason: an active goal there, a new item or ability that changes what you can do there, or passing through it on the way to somewhere unexplored. If you cannot state such a reason, choose a different action.
- FAMILIARITY LABELS: rooms, objects, and commands in this prompt carry a familiarity label based on what was actually tried and what it produced.
  - FRESH: little or nothing tried yet. Best default targets. Unexplored exits and untouched objects are where progress usually comes from. FRESH means unexplored, not safe. It never overrides a death warning or a recorded fatal action for this room. A general feeling that circumstances may have changed is not a reason to retry a command that killed you.
  - COVERED: already engaged and nothing new came up recently. Go back only with a specific purpose such as an agenda idea, a route waypoint, or a stored memory that names it. Going back just to look again is a wasted step.
  - EXHAUSTED: engaged many times with no score and no lasting change. Treat it as closed and spend steps elsewhere. Re-engage only if at least one of these applies: you carry a newly acquired item that is specifically relevant to the attempted interaction or stored goal, the current observation explicitly shows the target changed, or a route hint, known unearned reward, or active goal names it. An unrelated inventory change or a general feeling that something might have changed does not count.
  When nothing at your current location is FRESH or clearly purposeful, move toward the nearest FRESH room or the next hop of a route to an unearned reward.
- A movement command that keeps bouncing between the same two rooms with unchanged observations is a repeat loop even though the location changes each step. If the recent path shows such a cycle, mark is_repeat true and recommend a different exit or an interaction with an object instead.
---
...
=== KNOWN SCORING OPPORTUNITIES (FROM EARLIER EPOCHS) ===
{known_rewards}

=== OBJECT HISTORY NOTES ===
{object_history_notes}
...
=== RECENT PATH (last 8 location-command steps, with visit counts) ===
{recent_path}
...
2. Inventory reconciliation routing
   ...
   - Examples: an inventory listing appears; the object from Previous Action is
    ...
    - One observation may report both a possession change and an object-state
      change. Route each supported mechanism independently. Acquiring an object
      while turning it on, opening it, wearing it, or otherwise changing its
      state means inventory=true and world_state=true.
...
Mixed possession and state-change example:
Previous Action: activate lamp
Observation After Action: (Taken) The lamp is now on.
...
{{
  ...
  "brainstorm": true,
  "focus": ["lamp"],
  "note": "lamp was acquired and activated"
}}
|end|
...
evidence of a new room. Anchor on the descriptive sentences. Structural
differences in walls, exits, terrain, layout, or fixed landmarks can indicate
different rooms even with the same title. If the descriptions are essentially
the same text, they are almost certainly the same room. Prefer a new room when
the stable structural evidence differs materially or the evidence is
insufficient: a false merge poisons the map, while an extra split is recoverable.

Portable objects can be taken, dropped, opened, or moved, and transient events (sounds, weather, a passing animal) come and go between visits. Differences between the new description and a candidate's stored description that consist ONLY of such object lines or event lines appearing or disappearing are NOT evidence of a different room. Judge identity by the stable structure: the room's nature, its layout, and its exits.

If an object mentioned in a candidate's stored description is missing from the new description but that object is currently in YOUR inventory, you are the reason it is missing. This supports 'existing', not 'new'.
...
Your current inventory: {agent_inventory}
...
An observation may report a possession change and an object-state change at the
same time. Track those effects independently. Changing an object's state does
not mean it left inventory. Explicit acquisition evidence means the object was
added even when Previous Action was not an acquisition command.
...
- If the observation is vague and does not identify a concrete item, do not
  invent one.
- Set changed=true whenever authoritative=true or any of items_now_carried,
  items_added, or items_removed is non-empty. These fields must never contradict
  changed.
- evidence_quote must be an exact short quote copied from Observation After
  Action that supports the update. An authoritative empty items_now_carried
  list is allowed only when that quote explicitly says the player carries
  nothing.
...
{{
  ...
  "evidence_quote": "exact supporting words copied from the observation",
...
{{
  ...
  "items_removed": [],
  "evidence_quote": "(Taken)",
  "reason": "The lamp was acquired while also being turned on; activation does not remove it from carried inventory."
}}
|end|
...
{{
  ...
  "items_added": ["pile of leaves"],
  "items_removed": [],
  "evidence_quote": "You already have that!",
...
{{
  ...
  "items_added": [],
  "items_removed": ["lunch"],
  "evidence_quote": "You don't have that!",
...
- When Gate Routing Reason begins with "forced:first_confirmed_room_visit" or
  "forced:meaningfully_new_arrival_description", treat every concrete room
  object or fixed feature explicitly visible in the arrival observation as a
  newly observed KG object. Put those objects in new_objects even when the
  description does not say they just appeared. This is observation grounding,
  not an inferred world change.
...
Already Stored Situations contains every ordinary situation learned across all
epochs, including situations temporarily resolved in the current epoch. Before
returning a new record, compare the candidate against every stored record. If it
describes the same underlying object, obstacle, hazard, hidden feature, missing
condition, source location, or future-return problem, output none. A later
epoch, a more detailed observation, a changed possible solution, or different
wording does not make that underlying situation new.
...
- Compare the underlying hazard, blocker, source place, and route before adding.
  Different wording of the entry command does not make the same unresolved
  area a new situation.
...
- If Location Is Uncertain is True, Current Location is the last confirmed
  source room. Do not invent or return a synthetic destination name. Copy
  Current Location into "location" and describe that the problem was
  encountered after the supplied Entered Via command. A later observation may
  identify and resolve the destination.
...
Already Stored Situations: [{{"location": "Workshop", "situation": "sealed hatch blocks descent", "possible_solution": "an opening method may be needed"}}]
Observation: "The floor hatch is still sealed, although its locking wheel is now visible."
Output:
|start| none |end|
...
Current Location: {location}
Location Is Uncertain: {location_uncertain}
Entered Via: {entered_via}
Previous Action: {action}
Observation: {observation}
Current Inventory: {inventory}
...
A situation anchored to its last confirmed source room may describe an
unidentified destination entered by a command. It can be removed when Previous
Location matches that source, Previous Action matches the described entry
route semantically, and the current observation directly confirms safe arrival
or that the remembered hazard is no longer present. The stored location itself
does not need to equal Current Location in that case.
...
3. Active stored situations that may now be addressable because of the current room, inventory, or visible objects. Some were learned in earlier epochs and have reactivated because the game world reset; treat their compact facts as current guidance without inventing extra state.
4. Recent failed commands. If a command failed because the syntax was too specific, suggest simpler alternatives.
5. Pending carryover commands. Preserve still-useful pending ideas and propose alternatives when earlier ideas failed.
6. Recent same-location command outcomes. If several different commands in the same location produce similarly repeated, echoed, garbled, obscured, blocked, mismatched, or condition-dominated observations, consider whether a persistent environmental, perceptual, mental, or parser-like condition is interfering with normal command effects.
7. Tried Here is one combined list of commands already attempted at this location with outcomes and attempt counts. Do not re-propose EXHAUSTED commands. REOPENED commands may be reconsidered because an item named by that command or the named object's state changed. Unrelated inventory changes, new perspectives, and vague claims that the world may have changed do not justify repeating them. COVERED commands are prior evidence, not automatically forbidden. A verb that failed on one object can still work on another object.
8. Known Scoring Opportunities: if an unearned entry names the current room, propose its setup commands and its scoring command as important ideas. Reserve preparation fields for commands that establish a grounded resource; a scoring or direct setup command is not automatically preparation. If the current room appears on an unearned entry's route hint, movement along that route is justified.
   Before leaving the current room, compare every NOT EARNED entry's setup commands with Inventory and visible objects. If a setup command names a visible object that is not carried, propose acquiring that object as a high-priority missing setup resource before continuing toward the reward. The setup command is the grounding evidence; do not infer a requirement merely because an unrelated object seems useful.
9. Room Danger History: if prior deaths are recorded in this room, brainstorm preparation, defense, meaningfully different tactics, or retreat that are grounded in the observation, inventory, situations, or death evidence. Do not assume an unrecorded command is safe merely because it is absent from the fatal-command list.
10. Object History Notes: do not generate new command ideas for an object listed there unless inventory or the observation changed in a way you can name. Do not create preparation ideas whose only purpose is to unlock such an object.
11. TRAVERSAL FEATURES: if a visible object is a passage, staircase, door, path, opening, or similar traversal feature, include the movement command that uses it (for example west, up, enter passage) among the ideas.
12. FIRST VISIT SWEEP: on a room's first visit this epoch that still has untried described exits, at least one idea group must be about leaving through one of those exits.
13. ROUTE GUIDANCE: when the Route Guidance input names a command, include that command as a movement idea and give the stated reward as its reason.
14. FAMILIARITY RULES: prefer ideas about FRESH objects and FRESH or untried exits. Do not generate ideas that target EXHAUSTED rooms or objects unless the observation or the inventory provides one of the listed re-open reasons.
15. PREPARATION MATCHING: preparation establishes a resource before addressing a separate situation. Compare visible or carried objects against active situations, unresolved obstacles, and death lessons. Use `preparation_for` only when at least one command acquires, carries, equips, fills, loads, lights, or activates a grounded resource. Also set `preparation_resource` and `target_object` to that exact visible or carried resource, put the resource-establishing command first, and write `preparation_relation` as one concrete sentence explaining how the resource could functionally help with the separate hazard. Mere command wording is not evidence of a relationship: a coin is not a light source just because `light coin` is grammatical, and a book is not protection merely because `wear book` can be written. Commands that directly examine, search, open, attack, move, or otherwise manipulate the obstacle itself are ordinary interactions, not preparation.
...
- Use a list of objects. Each object must contain:
  ...
  - "target_object": for object ideas, the exact visible or carried object primarily targeted by the commands. Omit it for navigation or condition ideas.
  - "preparation_for": optional; use a short situation or hazard quote only when establishing a separate grounded resource prepares for it.
  - "preparation_resource": required whenever `preparation_for` is used; name the exact visible or carried resource acted on by the preparation command.
  - "preparation_relation": required whenever `preparation_for` is used; state the concrete functional relationship between that resource and the separate situation, grounded in the observation, memory, or ordinary object function.
...
- Preparation distinction: acquiring a grounded light source for a remembered dark area may use preparation fields because its ability to provide light is a plausible functional relation. Commands that directly pry, search, or open the blocker must omit them. If a separate key or tool is visible, acquiring that resource may be preparation when its functional relation to the situation is stated.
...
- Inventory is authoritative. Never assume an object is carried unless it appears in Inventory. If a visible object has a grounded purpose in a stored situation, death lesson, reward procedure, current obstacle, or clear reusable function, take it before relying on it; opening a container does not acquire its contents. Do not propose taking every visible object merely because it is available.
- If the observation reports carrying-capacity pressure, prioritize commands that make room for a grounded needed resource by comparing the purposes of carried items. Do not invent a future purpose for otherwise unsupported inventory.
- Do not propose acquiring an object already listed in Inventory. Compare the exact target of take/get/grab/acquire/collect/pick up commands against Inventory before returning the idea. If an idea's intended state already holds, treat it as completed rather than pending.
- Keep commands short and directly executable: "take lantern", "turn on lantern", "move rug", "look under rug".
- Do not re-propose a command listed in Tried Here unless the current state has meaningfully changed.
...
- VISITED-PLACES ADVISORY: before choosing a movement command, check where it leads and how many times you have already visited that room this epoch. If you have already been there several times, assume you have seen what it currently has to offer; another visit is low value, so prefer an unexplored exit or a room with an active goal instead. This is advice, not a ban. Returning to a much-visited room is justified when you can state a concrete reason: an active goal there, a new item or ability that changes what you can do there, or passing through it on the way to somewhere unexplored. If you cannot state such a reason, choose a different action.
- FAMILIARITY LABELS: rooms, objects, and commands in this prompt carry a familiarity label based on what was actually tried and what it produced.
  - FRESH: little or nothing tried yet. Best default targets. Unexplored exits and untouched objects are where progress usually comes from.
  - COVERED: already engaged and nothing new came up recently. Go back only with a specific purpose such as an agenda idea, a route waypoint, or a stored memory that names it. Going back just to look again is a wasted step.
  - EXHAUSTED: engaged many times with no score and no lasting change. Treat it as closed and spend steps elsewhere. Re-engage only if at least one of these applies: a newly carried item is specifically relevant to the attempted interaction or stored goal, the current observation explicitly shows the target changed, or a route hint, known unearned reward, or active goal names it. An unrelated inventory change or a general feeling that something might have changed does not count.
  When nothing at your current location is FRESH or clearly purposeful, move toward the nearest FRESH room or the next hop of a route to an unearned reward.
...
[
  {{
    ...
    "preparation_for": "dark upstairs area may require light",
    "preparation_resource": "lantern",
    "preparation_relation": "The lantern can provide light in the remembered dark area.",
...
Room Danger History: {room_danger_history}
Visited Places Advisory Data: {visit_advisory}
Relevant Lessons: {relevant_lessons}
Known Scoring Opportunities: {known_rewards}
Route Guidance: {route_guidance}
Untouched Objects: {untouched_objects}
Object History Notes: {object_history_notes}"""
