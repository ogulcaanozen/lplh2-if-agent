...
MOVEMENT_VERDICT_OVERRIDE = os.getenv(
    "LPLH_MOVEMENT_VERDICT_OVERRIDE", "true"
).lower() in ("1", "true", "yes", "on")
SAME_TITLE_CHAIN_SPLIT = os.getenv(
    "LPLH_SAME_TITLE_CHAIN_SPLIT", "true"
).lower() in ("1", "true", "yes", "on")
SAME_TITLE_CHAIN_MAX_MINTS_PER_EPOCH = int(
    os.getenv("LPLH_SAME_TITLE_CHAIN_MAX_MINTS_PER_EPOCH", "10")
)
EVAL_ENGINE_LOGGING = os.getenv(
    "LPLH_EVAL_ENGINE_LOGGING", "true"
).lower() in ("1", "true", "yes", "on")
ENGINE_LOCATION_GROUNDING = os.getenv(
    "LPLH_ENGINE_LOCATION_GROUNDING", "true"
).lower() in ("1", "true", "yes", "on")
ENGINE_INVENTORY_GROUNDING = os.getenv(
    "LPLH_ENGINE_INVENTORY_GROUNDING", "true"
).lower() in ("1", "true", "yes", "on")
...
ROOM_EXHAUSTED_VISITS = int(os.getenv("LPLH_ROOM_EXHAUSTED_VISITS", "5"))
ROOM_NOVELTY_STALE_VISITS = int(
    os.getenv("LPLH_ROOM_NOVELTY_STALE_VISITS", "2")
)
ROOM_PRIOR_EXHAUSTED_RECHECK_VISITS = int(
    os.getenv("LPLH_ROOM_PRIOR_EXHAUSTED_RECHECK_VISITS", "3")
)
OBJECT_EXHAUSTED_ATTEMPTS = int(
    os.getenv("LPLH_OBJECT_EXHAUSTED_ATTEMPTS", "5")
)
