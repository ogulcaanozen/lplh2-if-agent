...
class KGMap:
    ...
    def __init__(self, strict_location_authority: bool = False):
        ...
        self.engine_room_registry = {}  # persistent {Jericho object number: room evidence}
        self.engine_node_nums = {}  # epoch-local {room label: Jericho object number}
        ...
        for subj, rel, obj in triples:
            ...
            # Handle directional connections: <Location, direction, Destination>
                ...
                if loc:
                    ...
                    if direction not in self.nodes[loc].setdefault(
                        "described_directions", []
                    ):
                        self.nodes[loc]["described_directions"].append(direction)
        ...
        for subj, rel, obj in triples:
            ...
            if rel_lower == "have":
                ...
                if loc:
                    direction = self._canonical_direction(rel_lower)
                    node = self.nodes[loc]
                    if direction not in node.setdefault(
                        "described_directions", []
                    ):
                        node["described_directions"].append(direction)
    ...
    def arrival_edge_match(
        self,
        registry_candidates: list[tuple[str, dict]],
        from_location: str,
        action: str,
    ) -> tuple[str, str]:
        """Return the sole candidate matching a known arrival edge."""
        arrival_key = self._arrival_edge_key(from_location, action)
        if not arrival_key:
            return "", ""
        matches = []
        for registry_id, entry in registry_candidates or []:
            edges = {
                tuple(value)
                for value in (entry.get("arrival_edges", []) or [])
                if isinstance(value, (list, tuple)) and len(value) == 2
            }
            if arrival_key in edges:
                matches.append((registry_id, entry.get("label", "")))
    ...
    def mint_room(self, title: str, observation: str = "", epoch: int = 1,
        ...
        entry.setdefault("arrival_edges", [])
    ...
    def record_arrival_edge(
        self,
        destination: str,
        from_location: str,
        action: str,
    ) -> bool:
        """Persist one internal source-room/action arrival identity cue."""
        destination_id = self.registry_id_for(destination)
        entry = self.room_registry.get(destination_id)
        edge = self._arrival_edge_key(from_location, action)
        if not entry or not edge:
            return False
        edges = entry.setdefault("arrival_edges", [])
        encoded = [edge[0], edge[1]]
        if encoded in edges:
            return False
        edges.append(encoded)
        return True

    def _arrival_edge_key(
        self,
        from_location: str,
        action: str,
    ) -> tuple[str, str] | tuple:
        source_id = self.registry_id_for(from_location)
        action_key = re.sub(r"\s+", " ", str(action or "").strip().lower())
        if not source_id or not action_key:
            return ()
        direction = self._canonical_direction(action_key)
        edge_action = (
            direction
            if direction in self._standard_directions()
            else action_key
        )
        return source_id, edge_action
    ...
    def uncertain_location_label(self) -> str:
        """Return an epoch-local label without minting a persistent room."""
        source = self.current_location or "unknown"
        action = self.location_entered_dark_via or "unknown action"
        return f"Unseen area via {action} from {source}"
    ...
    def engine_num_for(self, location: str):
        """Return the Jericho room number bound to an epoch-local room node."""
        label = self._lookup_room(location)
        return self.engine_node_nums.get(label) if label else None

    def resolve_engine_location(self, engine_num, title: str = "",
                                engine_name: str = "", observation: str = "",
                                epoch: int = 1) -> tuple[str, str]:
        """Resolve a stable KG node from a Jericho room object number."""
        try:
            engine_num = int(engine_num)
        except (TypeError, ValueError):
            return "", ""

        existing = self.engine_room_registry.get(engine_num)
        if existing:
            registry_id = str(existing.get("registry_id", "") or "")
            label = self.adopt_registry_room(
                registry_id,
                observation=observation,
            )
            if not label:
                label = self._ensure_room(existing.get("label", ""))
                if registry_id:
                    self.node_registry_ids[label] = registry_id
            self.engine_node_nums[label] = engine_num
            return label, registry_id

        base = (
            canonical_room_display(engine_name)
            or canonical_room_display(title)
            or f"Room {engine_num}"
        )
        same_base_engine_rooms = [
            entry for number, entry in self.engine_room_registry.items()
            if number != engine_num
            and self._base_location_key(entry.get("base", ""))
            == self._base_location_key(base)
        ]
        label, registry_id = self.mint_room(
            base,
            observation=observation,
            epoch=epoch,
            force_new=bool(same_base_engine_rooms),
        )
        registry_entry = self.room_registry.get(registry_id, {})
        registry_entry["engine_location_num"] = engine_num
        registry_entry["engine_location_name"] = str(engine_name or "").strip()
        self.engine_room_registry[engine_num] = {
            "label": label,
            "base": base,
            "registry_id": registry_id,
    ...
    def set_engine_inventory(self, items: list[str]):
        """Replace inventory with Jericho's exact object-tree contents."""
        carried = []
        for raw_item in items or []:
            cleaned = self._clean_item_name(raw_item)
            if cleaned and cleaned not in carried:
                carried.append(cleaned)
        self.inventory = carried
        for item in carried:
            for node in self.nodes.values():
                node["have"] = [
                    obj for obj in node.get("have", [])
                    if not self._inventory_names_equivalent(item, obj)
                ]
                node["relations"] = [
                    rel for rel in node.get("relations", [])
                    if not self._inventory_names_equivalent(
                        item, rel.get("subject", "")
                    )
                    and not self._inventory_names_equivalent(
                        item, rel.get("object", "")
                    )
                ]

    def _inventory_names_equivalent(self, left: str, right: str) -> bool:
        left_clean = self._clean_item_name(left)
        right_clean = self._clean_item_name(right)
    ...
    def apply_inventory_update(self, update: dict, inventory_before: list[str] = None,
        ...
        original_changed = self._coerce_bool(update.get("changed", False))
        ...
        changed = bool(
            ...
            or result["items_added"]
            or result["items_removed"]
        )
        result["schema_repaired"] = bool(changed and not original_changed)
        if not changed:
            return result
        ...
        if result["authoritative"]:
            ...
            inventory_reference = before if inventory_before is None else inventory_before
            before_step = {
                self._clean_item_name(item)
                for item in inventory_reference
            }
            added_this_step = {
                self._clean_item_name(item)
                for item in before
    ...
    def mark_direction_tried(self, direction: str, message: str = ""):
        ...
        if self.current_location and self.current_location in self.nodes:
            ...
            if direction_lower and message:
                node.setdefault("blocked_direction_evidence", {})[
                    direction_lower
                ] = str(message).strip()[:300]
    ...
    def mark_direction_tried_at(self, direction: str, location: str,
        ...
        if location and location in self.nodes:
            ...
            if direction_lower in may:
                may.remove(direction_lower)
            if direction_lower in node["direction"] and not preserve_confirmed:
    ...
    def confirm_direction(self, from_location: str, direction: str, to_location: str,
        ...
        if from_location and from_location in self.nodes:
            ...
            node.setdefault("blocked_direction_evidence", {}).pop(
                direction_lower, None
            )
            self.record_arrival_edge(
                to_location,
                from_location,
                direction_lower,
            )
    ...
    def _split_contradictory_source_room(self, from_location: str,
        ...
        self.record_arrival_edge(to_location, sibling, direction)
        return {
            "epoch": int(epoch or 0),
            "step": int(step or 0),
            "from_room": from_location,
            ...
            "destination": to_location,
            "trigger": trigger,
            "moved_current_visit_blocked": [],
            "evidence": original.get(
                "blocked_direction_evidence", {}
            ).get(direction, ""),
        }

    def split_structural_block(
        self,
        from_location: str,
        direction: str,
        message: str,
        ...
        """Split a structural failure away from an identity with that exit."""
        from_location = self._canonicalize_known_location(from_location)
        direction = self._canonical_direction(direction)
        if (
            not from_location
            or from_location not in self.nodes
            or direction not in self.nodes[from_location].get("direction", {})
        ):
            return {}
        original = self.nodes[from_location]
        description = (
        ...
        sibling_node["arrival_description"] = description
        sibling_node["last_arrival_description"] = description
        if direction not in sibling_node.setdefault("blocked_directions", []):
            sibling_node["blocked_directions"].append(direction)
        sibling_node.setdefault("blocked_direction_evidence", {})[
            direction
        ] = str(message or "").strip()[:300]
        if direction in sibling_node.get("may_direction", []):
            sibling_node["may_direction"].remove(direction)
        self.current_location = sibling
        if sibling not in self.visited_rooms:
            self.visited_rooms.append(sibling)
        return {
            "epoch": int(epoch or 0),
            "step": int(step or 0),
            "from_room": from_location,
            ...
            "direction": direction,
            "destination": "",
            "trigger": "confirmed_exit_structural_failure",
            "evidence": str(message or "").strip()[:300],
        }

    def blocked_direction_message(self, location: str, direction: str) -> str:
        location = self._canonicalize_known_location(location)
        direction = self._canonical_direction(direction)
        if not location or location not in self.nodes:
            return ""
        return str(
            self.nodes[location]
            .get("blocked_direction_evidence", {})
            .get(direction, "")
        )
    ...
    def confirm_action_transition(self, from_location: str, action: str, to_location: str):
        ...
        self.record_arrival_edge(to_location, from_location, action_key)
    ...
    def get_current_room_info(self) -> dict:
        ...
        if self.location_uncertain:
            return {
                "location": self.uncertain_location_label(),
                "objects": [],
                "directions": {},
                "confirmed_actions": {},
                "may_direction": [],
    ...
    def to_prompt_string(self, active_goal_locations: list[str] = None,
                         familiarity_by_location: dict = None) -> str:
        ...
        return json.dumps(
            self.to_clean_dict(
                active_goal_locations=active_goal_locations,
                familiarity_by_location=familiarity_by_location,
            ),
            indent=2,
            ensure_ascii=False,
        )

    def to_clean_dict(self, active_goal_locations: list[str] = None,
                      familiarity_by_location: dict = None) -> dict:
        ...
        familiarity_by_location = familiarity_by_location or {}
        if self.location_uncertain:
            current_room_state = {
            ...
            for direction, destination in dict(
                ...
                exit_advisory[direction] = self._destination_visit_advisory(
                    destination, goal_keys, familiarity_by_location
                )
            ...
            described_exits = [
                direction
                for direction in current_node.get("described_directions", [])
                if (
                    direction not in current_node.get("direction", {})
                    and direction not in current_node.get(
                        "blocked_directions", []
                    )
                )
            ]
            current_familiarity = familiarity_by_location.get(
            ...
            current_room_state = {
                ...
                "described_exits_untried": described_exits,
                "needs_or_blockers": list(current_node.get("needs", []) or []),
                "visit_advisory": self._familiarity_advisory(
                    current_familiarity,
                    fallback_count=self.visit_counts.get(
        ...
        return {
            "current_location": prompt_location,
            "last_confirmed_location": (
                self.current_location if self.location_uncertain else ""
            ),
            ...
            "fresh_rooms": [
                ...
                for loc, data in self.nodes.items()
                if (
                    familiarity_by_location.get(loc, {}).get("tier") == "FRESH"
    ...
    def _destination_visit_advisory(self, destination: str,
                                    goal_keys: set[str],
                                    familiarity_by_location: dict) -> str:
        destination = str(destination or "").strip()
        familiarity = familiarity_by_location.get(destination, {})
        tier = familiarity.get("tier")
        count = int(self.visit_counts.get(destination, 0))
        status = (
            f"[{tier}]"
            if tier
            else (f"visited {count}x this epoch" if count else "unexplored")
        )
        rider = str(familiarity.get("rider") or "").strip()
        if rider:
            status += f" ({rider})"
    ...
    def _familiarity_advisory(
        self,
        familiarity: dict,
        fallback_count: int,
    ) -> str:
        tier = str(familiarity.get("tier") or "").strip()
        reason = str(familiarity.get("reason") or "").strip()
        rider = str(familiarity.get("rider") or "").strip()
        if not tier:
            return (
                f"your {self._ordinal(fallback_count)} visit this epoch"
            )
        detail = reason
        if rider:
            detail = f"{detail}; {rider}" if detail else rider
        return f"familiarity: {tier}" + (f" ({detail})" if detail else "")
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "engine_room_registry": copy.deepcopy(self.engine_room_registry),
            "engine_node_nums": dict(self.engine_node_nums),
            "location_uncertain": self.location_uncertain,
