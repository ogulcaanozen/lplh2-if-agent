"""LLM Client, a unified wrapper for action generation and experience summarization.
...
from .prompts import (
    ...
    PRECONDITION_HYPOTHESIS_PROMPT,
...
class LLMClient:
    ...
    def __init__(self, provider=None, model=None, temperature=None,
        ...
        self.last_precondition_prompt = None
        self.last_precondition_raw_response = None
        self.last_precondition_finish_reason = None
        ...
        self.last_blocked_classifier_prompt = None
        self.last_blocked_classifier_raw_response = None
        ...
        self.last_world_state_extraction_prompt = None
        self.last_world_state_extraction_raw_response = None
        self.last_world_state_extraction_finish_reason = None

        # qwen3 thinking-mode probe, Ollama only because qwen2.5 has no such mode.
    ...
    def summarize_experience(self, history: str, reward_change: int,
                              current_score: int, scoring_action: str = "",
                              location_issued: str = "",
                              location_after: str = "",
                              retry_note: str = "") -> str:
        ...
        prompt = EXPERIENCE_SUMMARIZATION_PROMPT.format(
            ...
            scoring_action=scoring_action or "unknown",
            location_issued=location_issued or "unknown",
            location_after=location_after or "unknown",
            retry_note=retry_note or "none",
        )
    ...
    def summarize_loss_experience(self, history: str, reward_change: int,
                                  current_score: int, fatal_action: str,
                                  location: str,
                                  location_issued: str = "",
                                  command_history_block: str = "",
                                  death_context: str = "") -> str:
        """Summarize a score loss/death with local attempt context.

        This uses the same LLM_es/fallback path as ordinary experience
        summarization, but the prompt asks the model to quote final evidence
        before drawing conclusions so cross-epoch death memories are cleaner.
        """
        prompt = LOSS_EXPERIENCE_SUMMARIZATION_PROMPT.format(
            history=history,
            reward_change=reward_change,
            current_score=current_score,
            fatal_action=fatal_action or "unknown",
            location=location or "unknown",
            location_issued=location_issued or location or "unknown",
            death_context=death_context or "Context: no additional grounded context.",
            command_history_block=(
                command_history_block or "No prior attempts recorded."
            ),
        )
        self.last_summary_kind = "score_loss"
        self.last_summary_prompt = prompt
        self.last_summary_raw_response = None
        if self._es_client and config.LLM_ES_MODEL:
            resp = self._es_client.chat.completions.create(
    ...
    def detect_stored_situation(self, location: str, action: str,
        ...
        prompt = STORED_SITUATION_DETECTION_PROMPT.format(
            ...
            location_uncertain=str(bool(location_uncertain)),
            entered_via=entered_via or "none",
        )
    ...
    def resolve_stored_situations(self, location: str,
                                  previous_location: str, action: str,
                                  observation: str, inventory: list,
    ...
    def resolve_location_identity(self, title: str, description: str,
        ...
        prompt = LOCATION_RESOLVER_PROMPT.format(
            ...
            agent_inventory=(
                ", ".join(str(item) for item in (agent_inventory or []))
                or "empty"
            ),
    ...
    def classify_blocked_message(self, direction: str, message: str) -> str:
        """Classify one contradictory blocked-exit message on demand."""
        prompt = (
            "You judge one line from a text adventure game that explains why "
            f"moving {direction} failed. Decide which class it is. structural "
            "means the exit simply does not exist there, for example: You "
            "can't go that way. conditional means an exit exists but an "
            "obstacle or state blocks it right now, for example: The window "
            "is closed. or: The rank undergrowth prevents eastward movement. "
            f"Message: {message} Answer with exactly one word: structural or "
            "conditional."
        )
        self.last_blocked_classifier_prompt = prompt
        self.last_blocked_classifier_raw_response = None
        try:
            if self._es_client and config.LLM_ES_MODEL:
                response, _ = self._chat_es_json(
                    prompt,
                    max_completion_tokens=32,
                    retry_tokens=32,
                    retry_instruction=(
                        "Answer with exactly one word: structural or conditional."
                    ),
                )
            else:
                response = self._chat_aux_fallback(prompt, max_new_tokens=32)
        except Exception:
            return "conditional"
        self.last_blocked_classifier_raw_response = response
        match = re.search(
            r"\b(structural|conditional)\b",
            str(response or ""),
            flags=re.IGNORECASE,
        )
        return match.group(1).lower() if match else "conditional"

    def hypothesize_precondition(self, death_count: int, fatal_action: str,
                                 hazard_location: str,
                                 death_observation: str, death_summary: str,
                                 inventory_at_death: list,
                                 previous_hypothesis: dict = None,
                                 previous_refutations: list = None) -> str:
        """Infer a non-local preparation requirement after repeated deaths."""
        prompt = PRECONDITION_HYPOTHESIS_PROMPT.format(
            death_count=int(death_count or 0),
            fatal_action=fatal_action or "unknown",
            hazard_location=hazard_location or "unknown",
            death_observation=death_observation or "",
            death_summary=death_summary or "No stored death summary text available.",
            inventory_at_death=json.dumps(
                inventory_at_death or [], ensure_ascii=False
            ),
            previous_hypothesis=json.dumps(
                previous_hypothesis or {}, ensure_ascii=False
            ),
            previous_refutations=json.dumps(
                previous_refutations or [], ensure_ascii=False
            ),
        )
        self.last_precondition_prompt = prompt
        self.last_precondition_raw_response = None
        self.last_precondition_finish_reason = None
        if self._es_client and config.LLM_ES_MODEL:
            response, finish_reason = self._chat_es_json(
                ...
                retry_instruction=(
                    "Return only the required |start|...|end| JSON object. "
                    "Use only the supplied repeated-death evidence."
                ),
                retry_tokens=1024,
            )
            self.last_precondition_finish_reason = finish_reason
        else:
            response = self._chat_aux_fallback(prompt, max_new_tokens=1024)
            self.last_precondition_finish_reason = "llm_a_qwen14b"

        self.last_precondition_raw_response = response
        match = re.search(r"\|start\|\s*(.*?)\s*\|end\|", response, re.DOTALL)
        return match.group(1).strip() if match else response.strip()

    def reconcile_inventory(self, location: str, action: str, action_valid,
                            command_outcome: dict, observation: str,
                            inventory_before: list, inventory: list,
                            visible_objects: list, gate_reason: str = "",
                            gate_focus: list = None) -> str:
        ...
        prompt = INVENTORY_RECONCILIATION_PROMPT.format(
            ...
            gate_focus=json.dumps(gate_focus or [], ensure_ascii=False),
        )

        self.last_inventory_reconciliation_prompt = prompt
        self.last_inventory_reconciliation_raw_response = None
        self.last_inventory_reconciliation_finish_reason = None
        if self._es_client and config.LLM_ES_MODEL:
            response, finish_reason = self._chat_es_json(
            ...
            self.last_inventory_reconciliation_finish_reason = finish_reason
        else:
            response = self._chat_aux_fallback(prompt, max_new_tokens=1024)
            self.last_inventory_reconciliation_finish_reason = "llm_a_qwen14b"

        self.last_inventory_reconciliation_raw_response = response
        m = re.search(r"\|start\|\s*(.*?)\s*\|end\|", response, re.DOTALL)
        return m.group(1).strip() if m else response.strip()

    def extract_world_state_update(self, location: str, action: str, action_valid,
                                   command_outcome: dict, observation: str,
                                   inventory: list, visible_objects: list,
                                   current_room_state: dict,
                                   gate_reason: str = "",
                                   gate_focus: list = None) -> str:
        """Extract durable object/world-state changes for the KG map.

        This intentionally uses LLM_a via the local fallback path, so the test
        version can evaluate Qwen-14B quality without spending o3-mini calls.
        """
        prompt = WORLD_STATE_EXTRACTION_PROMPT.format(
            location=location or "unknown",
            action=action or "none",
            action_valid=str(action_valid),
            command_outcome=json.dumps(command_outcome or {}, ensure_ascii=False),
            ...
            inventory=json.dumps(inventory or [], ensure_ascii=False),
            visible_objects=json.dumps(visible_objects or [], ensure_ascii=False),
            current_room_state=json.dumps(current_room_state or {}, ensure_ascii=False),
            gate_reason=gate_reason or "",
            gate_focus=json.dumps(gate_focus or [], ensure_ascii=False),
        )

        self.last_world_state_extraction_prompt = prompt
        self.last_world_state_extraction_raw_response = None
        self.last_world_state_extraction_finish_reason = None
        response = self._chat_aux_fallback(prompt, max_new_tokens=1024)
        self.last_world_state_extraction_finish_reason = "llm_a_qwen14b"
    ...
    def brainstorm_affordances(self, location: str, observation: str,
                               ...
                               known_rewards: str = "none known yet",
                               route_guidance: str = "none",
                               untouched_objects: list = None,
                               object_history_notes: str = "none",
        ...
        prompt = AFFORDANCE_BRAINSTORMING_PROMPT.format(
            ...
            stored_situations=json.dumps(stored_situations or [], ensure_ascii=False),
            visit_advisory=json.dumps(visit_advisory or {}, ensure_ascii=False),
            relevant_lessons=relevant_lessons or "none",
            known_rewards=known_rewards or "none known yet",
            route_guidance=route_guidance or "none",
            untouched_objects=json.dumps(
                untouched_objects or [], ensure_ascii=False
            ),
            object_history_notes=object_history_notes or "none",
            room_danger_history=(
                room_danger_history or "none recorded for current room"
            ),
            score=score,
        )
