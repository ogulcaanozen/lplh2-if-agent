...
def _world_object_by_num(env, object_num):
    if not object_num:
        return None
    try:
        getter = getattr(env, "get_object", None)
        if callable(getter):
            found = getter(int(object_num))
            if found is not None:
                return found
    except Exception:
        pass
    try:
        for obj in env.get_world_objects() or []:
            if _object_num(obj) == int(object_num):
                return obj
    except Exception:
        pass
    return None
...
def _object_name(obj) -> str:
    return re.sub(r"\s+", " ", str(getattr(obj, "name", "") or "")).strip()


def _engine_room_info(env) -> dict:
    """Read the player's stable top-level room from Jericho's object tree."""
    result = {
        "engine_location_available": False,
        "engine_location_num": None,
        "engine_location_name": "",
        "engine_enclosure_name": "",
        "engine_location_error": "",
    }
    try:
        room = env.get_player_location()
    except Exception as exc:
        result["engine_location_error"] = str(exc)
        return result
    if room is None:
        return result
    immediate = room
    ...
    while room is not None:
        ...
        if not parent_num:
            break
        parent = _world_object_by_num(env, parent_num)
        if parent is None:
            break
        room = parent
    ...
    if room_num is None:
        return result
    result.update({
        "engine_location_available": True,
        "engine_location_num": room_num,
        "engine_location_name": _object_name(room),
        "engine_enclosure_name": (
            _object_name(immediate) if _object_num(immediate) != room_num else ""
        ),
    })
    return result


def _inventory_descendants(env, obj, parent_num=None, depth: int = 0,
                           visited: set = None) -> list[dict]:
    """Flatten one carried object and its contained descendants."""
    visited = visited if visited is not None else set()
    number = _object_num(obj)
    if number is not None and number in visited:
        return []
    if number is not None:
        visited.add(number)
    records = []
    ...
    try:
        child_num = int(getattr(obj, "child", 0) or 0)
    except (TypeError, ValueError):
        child_num = 0
    sibling_seen = set()
    while child_num and child_num not in sibling_seen and len(visited) < 256:
        sibling_seen.add(child_num)
        child = _world_object_by_num(env, child_num)
        if child is None:
            break
        records.extend(_inventory_descendants(
            env,
            child,
            parent_num=number,
            depth=depth + 1,
        ...
        try:
            child_num = int(getattr(child, "sibling", 0) or 0)
        except (TypeError, ValueError):
            child_num = 0
    return records


def _engine_inventory_info(env) -> dict:
    """Read all possessed objects, including items in carried containers."""
    result = {
        "engine_inventory_available": False,
        "engine_inventory": [],
        "engine_inventory_objects": [],
        "engine_inventory_error": "",
    }
    try:
        roots = list(env.get_inventory() or [])
    except Exception as exc:
        result["engine_inventory_error"] = str(exc)
        return result
    records = []
    visited = set()
    for root in roots:
        records.extend(_inventory_descendants(env, root, visited=visited))
    names = []
    seen_names = set()
    for record in records:
        ...
        if key not in seen_names:
            ...
            seen_names.add(key)
    result.update({
        "engine_inventory_available": True,
        "engine_inventory": names,
        "engine_inventory_objects": records,
    })
    return result


def _engine_state_info(env) -> dict:
    """Collect supported Jericho state without making support mandatory."""
    state = {}
    if config.ENGINE_LOCATION_GROUNDING or config.EVAL_ENGINE_LOGGING:
        state.update(_engine_room_info(env))
    if config.ENGINE_INVENTORY_GROUNDING:
        state.update(_engine_inventory_info(env))
    state["eval_room_num"] = state.get("engine_location_num")
    state["eval_room_name"] = state.get("engine_location_name") or None
    return state


def _evaluation_room_info(env) -> dict:
    """Backward-compatible engine room fields used by evaluation logs."""
    state = _engine_room_info(env)
    return {
        "eval_room_num": state.get("engine_location_num"),
        "eval_room_name": state.get("engine_location_name") or None,
    }
...
class GameRunner:
    ...
    def run(self) -> dict:
        ...
        retrieval_log_path = os.path.join(self._experiment_log_dir, "retrieved_summaries_log.txt")
        ...
        self._retrieval_log_path = retrieval_log_path
        ...
        self._retrieval_log_file = open(retrieval_log_path, "w", encoding="utf-8", buffering=1)
        ...
        self._retrieval_log_file.write(f"LPLH2 Retrieved Summaries Log - {game_name}\n")
        self._retrieval_log_file.write(
            f"Epochs: {self.num_epochs} | Steps/epoch: {self.max_steps} | "
            "Records the summaries selected for the main action LLM at each step.\n"
        )
        self._retrieval_log_file.write("=" * 70 + "\n")
        self._attempt_ledger_log_file.write(f"LPLH2 Attempt Ledger Log - {game_name}\n")
        ...
        print(f"Retrieved summaries log: {retrieval_log_path}")
        ...
        try:
            for epoch in range(1, self.num_epochs + 1):
                ...
                print(
                    "  Reward Directory Entries: "
                    f"{epoch_result.get('reward_directory_entries', 0)}"
                )
            ...
            if self._action_generation_log_file:
                ...
                self._action_generation_log_file = None
            if self._retrieval_log_file:
                self._retrieval_log_file.close()
                self._retrieval_log_file = None
            ...
            print(f"  Retrieved summaries log: {self._retrieval_log_path}")
    ...
    def _compute_partial_summary(self, game_name: str, elapsed: float,
        ...
        return {
            ...
            "retrieval_log_path": self._retrieval_log_path,
    ...
    def _run_epoch(self, env, agent: LPLHAgent, epoch: int, game_name: str) -> tuple:
        ...
        engine_state = _engine_state_info(env)
        info.update(engine_state)
        initial_eval = {
            "eval_room_num": engine_state.get("eval_room_num"),
            "eval_room_name": engine_state.get("eval_room_name"),
        }
        if self._current_epoch_log is not None:
            self._current_epoch_log["initial_evaluation_room"] = initial_eval
            self._current_epoch_log["initial_engine_state"] = engine_state
        ...
        epoch_result = {
            ...
            "reward_directory_entries": stats.get(
                "reward_directory_entries", 0
            ),
            "game": game_name,
        }
    ...
    def _print_step_detail(self, agent: LPLHAgent, step: int, action: str,
        ...
        engine = modules.get("engine_grounding", {})
        if engine.get("source") == "jericho":
            grounded_parts = []
            if engine.get("location_available"):
                grounded_parts.append("location")
            if engine.get("inventory_available"):
                grounded_parts.append("inventory")
            print(
                f"  |     Jericho grounding: {', '.join(grounded_parts)}",
                flush=True,
            )
        ...
        world_state = modules.get("world_state_extraction", {})
        applied_world = world_state.get("applied_update", {})
        if world_state.get("applied") and applied_world:
            updates = applied_world.get("object_state_updates", [])
            new_objects = applied_world.get("new_objects", [])
            removed_objects = applied_world.get("removed_objects", [])
            parts = []
            if updates:
                first = updates[0]
                parts.append(f"{first.get('object', '?')}={first.get('state', '?')}")
            if new_objects:
                first = new_objects[0]
                parts.append(f"new {first.get('object', '?')}")
            if removed_objects:
                first = removed_objects[0]
                parts.append(f"removed {first.get('object', '?')}")
            if parts:
                print(f"  â”‚  World state update: {_trunc('; '.join(parts), 140)}",
                      flush=True)
    ...
    def _log_step(self, epoch: int, step: int, action: str, observation: str,
        ...
        lines.append(section("JERICHO STATE GROUNDING"))
        lines.append(json.dumps(
            modules.get("engine_grounding", {}), indent=2, ensure_ascii=False
        ))
        ...
        world_state = modules.get("world_state_extraction", {})
        lines.append(section("WORLD STATE EXTRACTION"))
        lines.append(f"Status: {world_state.get('status', 'missing')}")
        lines.append(f"Applied: {world_state.get('applied', False)}")
        lines.append(f"Reason: {world_state.get('reason', '')}")
        lines.append("\n[WORLD STATE RAW UPDATE]")
        lines.append(json.dumps(world_state.get("raw_update", {}), indent=2, ensure_ascii=False))
        lines.append("\n[WORLD STATE APPLIED UPDATE]")
        lines.append(json.dumps(world_state.get("applied_update", {}), indent=2, ensure_ascii=False))
    ...
    def _log_kg_location_step(self, epoch: int, step: int, detail: dict):
        ...
        metadata = {
            ...
            "engine_grounding": modules.get("engine_grounding", {}),
            "inventory_reconciliation": modules.get(
                "inventory_reconciliation", {}
            ),
            "room_familiarity": modules.get("room_familiarity", {}),
            "reward_directory": modules.get("reward_directory", {}),
            "extracted_you_in": [
    ...
    def _log_situation_memory_step(self, epoch: int, step: int, detail: dict):
        ...
        self._situation_log_file.write("\n\ngoal transitions this step:\n")
        self._situation_log_file.write(
            json.dumps(entry.get("goal_transitions", []), indent=2, ensure_ascii=False)
        )
        self._situation_log_file.write("\n\nall persistent goal situations:\n")
        self._situation_log_file.write(
            json.dumps(entry.get("goal_situations", []), indent=2, ensure_ascii=False)
        )
        self._situation_log_file.write("\n\nactive goal count:\n")
        self._situation_log_file.write(str(entry.get("active_goal_count", 0)))
    ...
    def _log_affordance_brainstorm_step(self, epoch: int, step: int, detail: dict):
        ...
        metadata = {
            ...
            "agenda_items_already_carried": entry.get(
                "agenda_items_already_carried", []
            ),
            "attempt_count_commands_here": len(entry.get("attempt_counts_here", {}) or {}),
        }
        ...
        self._affordance_log_file.write("\n\ntried-here context supplied to brainstorm:\n")
        self._affordance_log_file.write(str(entry.get("tried_here", "none")))
        ...
        self._affordance_log_file.write("\n\ncache status before brainstorm/merge:\n")
        self._affordance_log_file.write(
            json.dumps(entry.get("cache_status", {}), indent=2, ensure_ascii=False)
        )
        ...
        self._affordance_log_file.write(
            "\n\nagenda commands retired because target is already carried:\n"
        )
        self._affordance_log_file.write(
            json.dumps(
                entry.get("agenda_items_already_carried", []),
                indent=2,
                ensure_ascii=False,
            )
        )
        ...
        self._affordance_log_file.write("\n\npreparation validation decisions:\n")
        self._affordance_log_file.write(
            json.dumps(
                entry.get("preparation_validations", []),
                indent=2,
                ensure_ascii=False,
            )
        )
        self._affordance_log_file.write("\n\nagenda deduplication decisions:\n")
        self._affordance_log_file.write(
            json.dumps(
                entry.get("agenda_deduplication", []),
                indent=2,
                ensure_ascii=False,
            )
        )
    ...
    def _log_attempt_ledger_step(self, epoch: int, step: int, detail: dict):
        """Write factual per-room attempt ledger updates for every completed action."""
        if not self._attempt_ledger_log_file:
            return

        modules = detail.get("modules", {})
        entry = modules.get("attempt_ledger", {})
        if not entry:
            return
        ...
        metadata = {
            ...
            "count_after": entry.get("count_after"),
            "distinct_outcomes": entry.get("distinct_outcomes"),
            "room_visits_here": entry.get("room_visits_here"),
        }

        self._attempt_ledger_log_file.write("\n" + "=" * 90 + "\n")
        self._attempt_ledger_log_file.write(
            f"[EPOCH {epoch} | STEP {step:03d}] "
            f"{metadata.get('command')} -> {metadata.get('outcome_class')}\n"
        )
        self._attempt_ledger_log_file.write("-" * 90 + "\n")
        self._attempt_ledger_log_file.write("metadata:\n")
        self._attempt_ledger_log_file.write(json.dumps(metadata, indent=2, ensure_ascii=False))
        self._attempt_ledger_log_file.write("\n\nledger record after step:\n")
        self._attempt_ledger_log_file.write(
            json.dumps(entry.get("record", {}), indent=2, ensure_ascii=False)
        )
        self._attempt_ledger_log_file.write("\n\ncurrent room visit update:\n")
        self._attempt_ledger_log_file.write(
            json.dumps(entry.get("current_room_visit"), indent=2, ensure_ascii=False)
        )
        self._attempt_ledger_log_file.write("\n\nroom context after step:\n")
        self._attempt_ledger_log_file.write(str(entry.get("room_context_after", "")))
        self._attempt_ledger_log_file.write("\n\nobject target and futility update:\n")
        self._attempt_ledger_log_file.write(
            json.dumps(
                modules.get("interaction_stats", {}),
                indent=2,
                ensure_ascii=False,
            )
        )
    ...
    def _write_action_generation_record(self, epoch: int, step: int, label: str,
        ...
        if not generation:
            return
        self._write_retrieved_summaries_record(
            epoch=epoch,
            step=step,
            label=label,
            command=command,
        ...
        metadata = {
            ...
            "agenda_selection_audit": generation.get(
                "agenda_selection_audit", {}
            ),
            "acquisition_choice_audit": generation.get(
                "acquisition_choice_audit", {}
            ),
            "blocked_direction_guard": generation.get("blocked_direction_guard", {}),
            "command_reopen_audit": generation.get("command_reopen_audit", []),
        ...
        self._action_generation_log_file.write("\n\nroom danger history:\n")
        self._action_generation_log_file.write(
            str(generation.get("room_danger_history", "none recorded for current room"))
        )
        ...
        self._action_generation_log_file.write("\n\ncommand-memory reopen audit:\n")
        self._action_generation_log_file.write(
            json.dumps(
                generation.get("command_reopen_audit", []),
                indent=2,
                ensure_ascii=False,
            )
        )
        ...
        self._action_generation_log_file.write("\n\nagenda selection audit:\n")
        self._action_generation_log_file.write(
            json.dumps(
                generation.get("agenda_selection_audit", {}),
                indent=2,
                ensure_ascii=False,
            )
        )
        self._action_generation_log_file.write("\n\nacquisition choice audit:\n")
        self._action_generation_log_file.write(
            json.dumps(
                generation.get("acquisition_choice_audit", {}),
                indent=2,
                ensure_ascii=False,
            )
        )
    ...
    def _write_retrieved_summaries_record(self, epoch: int, step: int, label: str,
                                          command: str, generation: dict,
                                          observation: str, score):
        """Write a focused audit log of memories selected for the action prompt."""
        if not self._retrieval_log_file or not generation:
            return

        retrieval_debug = generation.get("retrieval_debug") or {}
        state_snapshot = generation.get("state_snapshot_at_generation") or {}
        metadata = {
            "step": step,
            "label": label,
            "command": command or generation.get("parsed_command"),
            "parsed_command": generation.get("parsed_command"),
            "score": score,
            "score_at_generation": generation.get("score_at_generation"),
            "location_at_generation": state_snapshot.get("location"),
            "selection_policy": retrieval_debug.get("selection_policy"),
            "diversity_enabled": retrieval_debug.get("diversity_enabled"),
            "top_k": retrieval_debug.get("top_k"),
            "fetch_k": retrieval_debug.get("fetch_k"),
            "earned_score_event_keys_this_epoch": retrieval_debug.get(
        ...
        self._retrieval_log_file.write("-" * 90 + "\n")
        self._retrieval_log_file.write("metadata:\n")
        self._retrieval_log_file.write(json.dumps(metadata, indent=2, ensure_ascii=False))
        self._retrieval_log_file.write("\n\ncurrent observation:\n")
        self._retrieval_log_file.write(str(observation or ""))
        self._retrieval_log_file.write("\n\nselected summaries shown to main LLM:\n")
        self._retrieval_log_file.write(
            json.dumps(retrieval_debug.get("shown", []), indent=2, ensure_ascii=False)
        )
        self._retrieval_log_file.write("\n\ncandidate summaries considered:\n")
        self._retrieval_log_file.write(
            json.dumps(retrieval_debug.get("candidates", []), indent=2, ensure_ascii=False)
        )
        self._retrieval_log_file.write("\n\nretrieved experiences text sent to main LLM:\n")
        self._retrieval_log_file.write(str(generation.get("retrieved_experiences", "")))
        self._retrieval_log_file.write("\n\nraw retrieval debug:\n")
        self._retrieval_log_file.write(
            json.dumps(retrieval_debug, indent=2, ensure_ascii=False)
        )
        self._retrieval_log_file.write("\n" + "=" * 90 + "\n")
    ...
    def _save_results(self, results: dict, game_name: str):
        ...
        results["retrieval_log_path"] = self._retrieval_log_path
