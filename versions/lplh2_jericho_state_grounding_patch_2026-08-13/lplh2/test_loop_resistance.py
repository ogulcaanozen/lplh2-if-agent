...
class LoopResistanceTests(unittest.TestCase):
    def test_command_equivalence_and_agenda_consumption(self):
        ...
        self.assertEqual(agenda[0]["pending_commands"], [])
        self.assertEqual(agenda[0]["agenda_status"], "already_done")
        self.assertEqual(
            agenda[0]["already_done"][0]["command"],
            "climb up tree",
        )
    ...
    def test_preparation_idea_ranks_first_and_survives_state_change(self):
        ...
        preparation = {
            ...
            "target_object": "protective coat",
            "preparation_for": "hazardous passage requires protection",
            "preparation_resource": "protective coat",
            "preparation_relation": "The coat can provide the protection required by the passage.",
    ...
    def test_carried_acquisition_is_already_done_without_substring_match(self):
        brainstormer = AffordanceBrainstormer()
        ideas = [{
            "location": "Room",
            "situation": "water and a bottle are nearby",
            "commands_to_try": [
                "take bottle",
                "take water",
                "examine bottle",
        ...
        agenda = brainstormer.build_agenda(
            ideas,
            inventory=["bottle", "water bottle"],
        )
        self.assertEqual(
            agenda[0]["pending_commands"],
            ["take water", "examine bottle"],
        )
        self.assertEqual(
            agenda[0]["already_done"][0]["command"],
            "take bottle",
        ...
        self.assertEqual(
            brainstormer.last_inventory_completions[0]["reason"],
            "agenda_item_already_carried",
        )
    ...
    def test_uncertain_situation_is_grounded_to_entry_gateway(self):
        ...
        self.assertIn("after 'down' from Landing", stored["situation"])
        self.assertIn(
