...
class PromptSlimTests(unittest.TestCase):
    def test_main_prompt_has_one_tried_here_and_new_sections(self):
        prompt = LPLH_ACTION_GENERATION_PROMPT.format(
            ...
            action_pairs="none",
            experiences="none",
            room_danger_history="none recorded for current room",
    ...
    def test_real_tried_context_merges_ledger_and_exact_state(self):
        ...
        context = agent._tried_here_context("Room", fingerprint, snapshot)
        self.assertEqual(context.count("open door [COVERED]"), 1)
    ...
    def test_kg_context_has_compact_frontier_and_no_local_relations(self):
        ...
        context = kg.to_clean_dict(
            familiarity_by_location={
                "Room A": {"tier": "FRESH", "reason": "first visit"},
                "Room B": {"tier": "FRESH", "reason": "unvisited"},
            }
        )
        self.assertNotIn("frontier", context)
        self.assertNotIn("rooms_with_unexplored_exits", context)
        self.assertEqual(context["fresh_rooms"], ["Room B"])
    ...
    def test_brainstorm_template_formats_with_consolidated_inputs(self):
        prompt = AFFORDANCE_BRAINSTORMING_PROMPT.format(
            ...
            known_rewards="none known yet",
            route_guidance="none",
            untouched_objects=json.dumps(["door"]),
            object_history_notes="none",
            room_danger_history="none recorded for current room",
        )
        ...
        self.assertIn("FAMILIARITY LABELS", prompt)

    def test_familiarity_legend_appears_in_both_templates(self):
        self.assertIn("FAMILIARITY LABELS", LPLH_ACTION_GENERATION_PROMPT)
        self.assertIn("FAMILIARITY LABELS", AFFORDANCE_BRAINSTORMING_PROMPT)
