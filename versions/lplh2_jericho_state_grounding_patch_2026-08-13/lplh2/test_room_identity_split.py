...
class RoomIdentitySplitTests(unittest.TestCase):
    ...
    def test_intervening_same_room_state_change_suppresses_split(self):
        agent = LPLHAgent.__new__(LPLHAgent)
        agent._room_state_change_steps = {}
        self.assertFalse(
            agent._same_room_state_changed_after("Forest", 3)
        )
        agent._room_state_change_steps["forest"] = 4
        self.assertTrue(
            agent._same_room_state_changed_after("Forest", 3)
        )
        ...
        split = kg.confirm_direction(
            ...
            epoch=1,
            step=5,
            allow_split=not agent._same_room_state_changed_after(
                forest, 3
            ),
        )
    ...
    def test_without_state_change_blocked_then_open_still_splits(self):
        agent = LPLHAgent.__new__(LPLHAgent)
        agent._room_state_change_steps = {}
        kg, forest, clearing = self._map()
        kg.mark_direction_tried_at(
        ...
        self.assertIn("new_sibling", split)
