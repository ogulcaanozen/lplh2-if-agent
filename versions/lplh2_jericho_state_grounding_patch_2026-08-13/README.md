...
This snapshot is `lplh2_jericho_state_grounding_patch_2026-08-13`. It is an
isolated child of `lplh2_reward_resource_inventory_situation_patch_2026-08-02`.

New in this version:

- supported games use Jericho room object numbers as stable KG identity;
- supported games use Jericho's recursive carried-object tree as inventory;
- grounded state reaches the gate and all KG-backed prompts before decisions;
- unsupported APIs retain the prior LLM-based behavior independently;
- grounding decisions and authoritative inventory changes are logged;

Inherited from the parent snapshot:

- unearned reward setup commands are compared with visible, uncarried resources
  by the existing main and brainstorm LLM calls;
- mixed acquisition and object-state observations are demonstrated to the
  existing auxiliary gate and inventory reconciler;
- ordinary situations retain creation epoch and step internally while prompts
  keep the same compact three-field representation;
- situation detection compares against the full persistent situation store so
  inherited or temporarily resolved problems are not re-added as new records;

- parser-rejected object commands are grounded into the existing interaction
  tracker through agenda, FM, or visible-object evidence;
- five distinct no-progress commands can exhaust an object, while score,
  lasting state change, and new information protect productive objects;
- preparation priority requires a grounded resource and an establishing
  command; malformed claims are logged and demoted;
- equivalent pending-command sets merge before the five-entry agenda cap;
- action logs record agenda rank, command position, and skipped grounded ideas;

- confirmed movement remains available for transit through exhausted rooms;
- titleless unclear non-movement observations cannot create false room
  identities from state-preserving look probes;
- uncertain destination situations use a confirmed source room and entry
  command instead of a permanent synthetic location;
- meaningful room arrivals force semantic extraction of explicitly visible
  objects even when the auxiliary gate skips world-state extraction;

- accepted direction commands can override an erroneous auxiliary `moved=no`
  verdict when score or grounded arrival-description evidence confirms movement;
- consecutive same-title movement can mint bounded sibling rooms, allowing
  chains such as physically distinct Detective hallways to remain traversable;
- intervening same-room state changes prevent blocked/open edge evidence from
  causing a false room split;
- death-warning prompt rules explicitly outrank FRESH familiarity and known
  reward temptation without adding a deterministic action veto;
- known rewards are ranked by graph distance, nearby side rewards are shown,
  registry-identical reward copies are merged, and rendering uses an adaptive
  ten-line cap instead of a fixed six-entry slice;
- the three existing attempt stores render through one state-aware command view:
  COVERED, EXHAUSTED, or REOPENED;
- retry fingerprints intentionally contain location, inventory, and visible
  object state, but exclude observation narration and score;
