...
- **Tried Here**: one combined list shows every command already attempted at this location with its outcome and attempt count. COVERED means it has been tried but is not established as exhausted. EXHAUSTED means it has repeatedly produced no movement, score, or lasting state change under the same relevant world conditions; choose something else. REOPENED means a relevant inventory item or the named object's state changed since those attempts, so retrying may be reasonable but is not mandatory. Narrative wording or score changes alone do not reopen a command. Any command not listed has never been tried here and is your best default candidate. When nothing else decides it, prefer in this order: unlisted, then REOPENED, then COVERED, then EXHAUSTED last. Never use an EXHAUSTED command as a setup step for another command.
...
7. Tried Here is one combined list of commands already attempted at this location with outcomes and attempt counts. Do not re-propose EXHAUSTED commands. REOPENED commands may be reconsidered because a relevant inventory item or the named object's state changed. COVERED commands are prior evidence, not automatically forbidden. A verb that failed on one object can still work on another object.
