...
This snapshot is `lplh2_purpose_death_situation_patch_2026-08-01`. It is an
isolated child of `lplh2_agenda_grounding_patch_2026-07-31`.

New in this version:

- acquisition is purpose-aware rather than driven by visibility alone;
- preparation priority requires an LLM-written functional relation between a
  grounded resource and a separate hazard;
- accumulated deaths are shown as room-level danger evidence to both the
  brainstormer and final action selector;
- ordinary situations persist across epochs, while successful resolution
  suppresses them only for the current epoch;
- logs expose acquisition purpose, preparation validation, room danger, and
  situation persistence state;
