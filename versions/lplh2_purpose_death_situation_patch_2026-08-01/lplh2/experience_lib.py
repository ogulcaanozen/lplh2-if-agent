...
class ExperienceLib:
    ...
    def store_experience(self, experience_text: str, metadata: dict = None):
        ...
        if str(metadata.get("kind") or "") == "route":
            lesson_signature = self._lesson_signature(experience_text)
            location = self._normalize_piece(
                metadata.get("location_issued")
                or metadata.get("location")
                or ""
            )
            lesson_key = f"{location}|route|{lesson_signature}"
            if lesson_signature:
                metadata["lesson_signature"] = lesson_signature
                metadata["lesson_location_key"] = location
            if lesson_signature and (
                lesson_key in self._lesson_keys
                or self._stored_lesson_exists(
                    location=location,
                    kind="route",
                ...
                logger.info(
                    "Skipped duplicate route experience "
                    f"(total skips: {self.duplicate_skip_count})"
                )
                return
            if lesson_signature:
                self._lesson_keys.add(lesson_key)
        ...
        try:
            ...
            chroma_metadata = self._sanitize_chroma_metadata(metadata)
    ...
    def _stored_lesson_exists(self, location: str, kind: str,
                              lesson_signature: str) -> bool:
        try:
            self._init_chroma()
            ...
            for metadata in metadatas or []:
                stored_location = self._normalize_piece(
                    metadata.get("location_issued")
                    or metadata.get("location")
                    or ""
                )
            ...
            logger.debug(f"Route lesson dedup lookup failed: {exc}")
        return False

    @classmethod
    def _lesson_signature(cls, experience_text: str) -> str:
        ...
        return cls._normalize_piece(lesson or text)

    @staticmethod
    def _normalize_piece(value) -> str:
    ...
    def retrieve_relevant(self, query: str, top_k: int = None) -> str:
        ...
        # Guard against empty DB, checking ChromaDB rather than the in-memory list.
