...
- **Preparation Agenda**: If the affordance agenda contains a PREPARATION item that links an object in this room to a known situation or hazard, acquiring that object takes priority over leaving the room. Take it first, then continue.
...
8. Known Scoring Opportunities: if an unearned entry names the current room, propose its setup commands and its scoring command as high priority ideas and set preparation_for to a short quote of that entry. If the current room appears on an unearned entry's route hint, movement along that route is justified.
...
14. PREPARATION MATCHING: compare the objects visible or takeable in the current observation against the active stored situations, unresolved obstacles, and any death lessons shown to you. If an object could plausibly resolve one of them (for example a light source for a dark area, a tool for a blocked passage, a key for a locked thing), emit `take <object>` as a command idea, set the field `preparation_for` to a short quote of the situation it addresses, and treat it as high priority.
...
- Use a list of objects. Each object must contain:
  ...
  - "preparation_for": optional; use a short situation or hazard quote only when acquiring the proposed object prepares for it.
