...
class AttemptLedger:
    ...
    def record_step(self, location: str, command: str, observation: str,
                    ...
                    state_key: str,
                    step: int, epoch: int = 1,
                    world_fingerprint: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        fingerprint = self._clean_world_fingerprint(world_fingerprint)
        if fingerprint:
            fingerprints = list(record.get("state_fingerprints", []))
            fingerprint_key = self._world_fingerprint_key(fingerprint)
            if fingerprint_key and all(
                self._world_fingerprint_key(value) != fingerprint_key
                for value in fingerprints
            ):
                fingerprints.append(fingerprint)
                fingerprints = fingerprints[-8:]
            record["state_fingerprints"] = fingerprints
    ...
    @staticmethod
    def _clean_world_fingerprint(value: Any) -> dict[str, Any]:
        """Keep only the stable pre-command world fields used for retry tiers."""
        if not isinstance(value, dict):
            return {}
        location = clean_text(value.get("location")) or "unknown"
        inventory = sorted({
            normalize_text(item)
            for item in (value.get("inventory") or [])
            if normalize_text(item)
        })
        raw_states = value.get("object_states") or {}
        object_states = {}
        if isinstance(raw_states, dict):
            for name, state in raw_states.items():
                clean_name = normalize_text(name)
                if clean_name:
                    object_states[clean_name] = normalize_text(state)
        return {
            "location": location,
            "inventory": inventory,
    ...
    @staticmethod
    def _world_fingerprint_key(value: Any) -> str:
