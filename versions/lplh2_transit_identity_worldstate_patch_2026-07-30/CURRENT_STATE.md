...
This folder is the transit, identity, and arrival-world-state experiment:

```text
versions/lplh2_transit_identity_worldstate_patch_2026-07-30
```

**Status: experimental child of the current main LPLH2 thesis/testing baseline.**

It is built on:

```text
versions/lplh2_grounding_consistency_patch_2026-07-29
```

## Included

- Confirmed KG or ledger movement remains usable as transit even when its
  command or destination is marked `EXHAUSTED`; the label remains advisory.
- Active-goal preparation and blocked-direction checks still apply to movement.
- Titleless, unclear, non-movement observations retain the current room instead
  of invoking same-title identity resolution.
- First confirmed room visits and meaningfully new arrival descriptions force
  the existing LLM world-state extractor even when the auxiliary gate skips it.
- Uncertain destination situations persist under the last confirmed source
  room and retain the entry command in their text; synthetic prompt labels are
  never canonical memory locations.
- Situation resolution receives the previous confirmed location so a later
  grounded safe arrival can resolve a source-anchored hazard.

Inherited from the parent snapshot:

- Grounded movement override for erroneous auxiliary no-move verdicts.
- Bounded same-title chain splitting with exact-signature reuse first.
- State-gated blocked/open contradiction repair.
- Prompt-only death-warning precedence over familiarity labels.
- Distance-ranked and adaptively rendered reward opportunities.
- Explicit nearby side-reward context.
- Registry-scoped reward deduplication.
- State-aware COVERED, EXHAUSTED, and REOPENED command tiers backed by the
  existing ledger, failure memory, and same-state memory.
- Regression coverage increased to 129 passing tests.

Inherited from the main baseline:

- Structural blocked/open contradictions split merged same-title rooms only
  after a lazy auxiliary classification.
- Persistent arrival-edge evidence breaks otherwise ambiguous same-title
  identity ties without entering prompts.
- Room, object, and command familiarity uses FRESH, COVERED, and EXHAUSTED
  advisory labels with reward-route and active-goal exemptions.
- First-visit described exits and untouched objects receive explicit
  brainstorming attention.
- Reward routes store structured hops, expose the next hop, and retain
  recoverable failure annotations.
- Route memories are stored and retrieved by their source room.
- Exact-room achievement, enabler, and route records receive filtered local
  fetches before global semantic retrieval.
- Causal setup filtering removes movement and observation-only commands.
- Retrieval diagnostics record local fetch counts and drop reasons.
- Generic condition carryover expires across room changes unless the current
  room has an active condition situation.
...
- Reward-aware brainstorming with high-value retrieved lessons only.
- Route-summary deduplication at storage and retrieval.
- Compact KG frontier and six-turn main action history.
...
The root convenience notebook and this version's notebook point to this
experimental folder.
