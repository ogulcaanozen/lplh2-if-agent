...
class AffordanceBrainstormer:
    ...
    def cached_ideas_for_state(self, location: str,
        ...
        if cached.get("state_key") != state_key:
            ideas = self._ideas_still_relevant(ideas, state_signature)
        if failed_commands:
    ...
    def _ideas_still_relevant(self, ideas: list[dict[str, Any]],
                              state_signature: dict[str, Any] | None) -> list[dict[str, Any]]:
        if not ideas:
            return []
        visible = set((state_signature or {}).get("visible_objects", []) or [])
        inventory = set((state_signature or {}).get("inventory", []) or [])
        anchors = {item for item in visible.union(inventory) if item}
        ...
        for idea in ideas:
            ...
            if any(anchor and anchor in haystack for anchor in anchors):
                output.append(idea)
        return output[:5]
    ...
    def build_agenda(self, ideas: list[dict[str, Any]],
        ...
        for idea in pending_ideas:
            ...
            for command in pending:
                ...
                ledger = attempt_counts.get(cmd_key, {})
                count = int(ledger.get("count", 0)) if ledger else 0
                if count:
                    tried_annotations.append({
                        "command": command,
                        "tried_count": count,
                        "last_outcome": ledger.get("last_outcome", ""),
                        "distinct_outcomes": ledger.get("distinct_outcomes", 0),
                        "last_step": ledger.get("last_step"),
                    })
                annotated_pending.append({
                    "command": command,
                    "tried_count": count,
                    "last_outcome": ledger.get("last_outcome", "") if ledger else "",
                })
            entry = {
                "location": idea.get("location", "current location"),
                "situation": idea.get("situation", ""),
                "pending_commands": pending,
                "command_statuses": annotated_pending,
            }
            if idea.get("kind"):
            ...
            if tried_annotations:
                entry["already_attempted_in_room"] = tried_annotations
            if pending and all(item.get("tried_count", 0) > 0 for item in annotated_pending):
                entry["agenda_status"] = "all_commands_tried"
            elif any(item.get("tried_count", 0) > 0 for item in annotated_pending):
