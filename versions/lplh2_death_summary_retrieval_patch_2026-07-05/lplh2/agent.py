...
class LPLHAgent:
    ...
    def act(self, observation: str, score: int, done: bool, info: dict,
        ...
        if experience_triggered:
            try:
                ...
                score_location = self.kg_map.current_location or "unknown"
    ...
    def _select_diverse_experiences(self, records: list[dict],
        ...
        def add(record: dict) -> bool:
            nonlocal route_for_current_room
            ...
            if kind_counts.get(kind, 0) >= 2:
                return False
            if kind == "route" and self._route_for_current_room(record):
                if route_for_current_room >= 1:
                    return False
                route_for_current_room += 1
            selected.append(record)
            selected_ids.add(rid)
            kind_counts[kind] = kind_counts.get(kind, 0) + 1
            return True

        # Reserve one slot for a non-route memory when possible.
