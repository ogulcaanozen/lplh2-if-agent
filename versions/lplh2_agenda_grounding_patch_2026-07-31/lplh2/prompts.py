...
9. Object History Notes: do not generate new command ideas for an object listed there unless inventory or the observation changed in a way you can name. Do not create preparation ideas whose only purpose is to unlock such an object.
10. TRAVERSAL FEATURES: if a visible object is a passage, staircase, door, path, opening, or similar traversal feature, include the movement command that uses it (for example west, up, enter passage) among the ideas.
11. FIRST VISIT SWEEP: on a room's first visit this epoch that still has untried described exits, at least one idea group must be about leaving through one of those exits.
12. ROUTE GUIDANCE: when the Route Guidance input names a command, include that command as a movement idea and give the stated reward as its reason.
13. FAMILIARITY RULES: prefer ideas about FRESH objects and FRESH or untried exits. Do not generate ideas that target EXHAUSTED rooms or objects unless the observation or the inventory provides one of the listed re-open reasons.
14. PREPARATION MATCHING: preparation establishes a resource before addressing a separate situation. Compare visible or carried objects against active situations, unresolved obstacles, and death lessons. Use `preparation_for` only when at least one command acquires, carries, equips, fills, loads, lights, or activates a grounded resource. Also set `preparation_resource` and `target_object` to that exact visible or carried resource, and put the resource-establishing command first. Commands that directly examine, search, open, attack, move, or otherwise manipulate the obstacle itself are ordinary interactions, not preparation.
...
- Preparation distinction: `take lantern` for a remembered dark area may use preparation fields; `pry door`, `search door`, or `open door` directly addresses the blocker and must omit them. If a separate key or tool is visible, acquiring that resource may be preparation even when the remembered situation also names it.
