...
14. PREPARATION MATCHING: preparation establishes a resource before addressing a separate situation. Compare visible or carried objects against active situations, unresolved obstacles, and death lessons. Use `preparation_for` only when at least one command acquires, carries, equips, fills, loads, lights, or activates a grounded resource. Also set `preparation_resource` to that exact visible or carried object. Commands that directly examine, search, open, attack, move, or otherwise manipulate the obstacle itself are ordinary interactions, not preparation.
