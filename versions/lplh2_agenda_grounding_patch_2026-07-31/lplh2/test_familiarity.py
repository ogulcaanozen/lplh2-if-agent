...
class FamiliarityTests(unittest.TestCase):
    ...
    def test_late_novelty_expires_after_visit_gap(self):
        rooms = RoomFamiliarity()
        for _ in range(4):
            rooms.visit("r1", "Tree", 1)
        rooms.record_novelty("r1", "Tree", "reward")
        rooms.visit("r1", "Tree", 1)
        self.assertEqual(rooms.tier("r1")["tier"], "COVERED")
        rooms.visit("r1", "Tree", 1)
        tier = rooms.tier("r1")
        self.assertEqual(tier["tier"], "EXHAUSTED")
        self.assertEqual(tier["visits_since_novelty"], 2)

    def test_new_late_novelty_resets_exhaustion_gap(self):
        rooms = RoomFamiliarity()
        for _ in range(10):
            rooms.visit("r1", "Tree", 1)
        rooms.record_novelty("r1", "Tree", "new_visible_object")
        self.assertEqual(rooms.tier("r1")["tier"], "COVERED")
        rooms.visit("r1", "Tree", 1)
        self.assertEqual(rooms.tier("r1")["tier"], "COVERED")
        rooms.visit("r1", "Tree", 1)
    ...
    def test_command_exhausted_tag(self):
        ...
        for step in range(1, 4):
            agent.attempt_ledger.record_step(
                ...
                "same", step, 1, world_fingerprint=fingerprint,
            )
        text = agent._tried_here_context("Room", fingerprint, snapshot)
