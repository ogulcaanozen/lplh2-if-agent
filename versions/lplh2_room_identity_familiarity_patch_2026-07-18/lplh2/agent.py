...
class LPLHAgent:
        ...
        if pure_rejection or self.kg_map.location_uncertain:
            ...
            try:
                ...
                if ((action_valid is True or gate_accepted)
                    ...
                    blocked_class = ""
                    if blocked_message:
                        blocked_class = self.llm.classify_blocked_message(
                            movement_direction,
                            blocked_message,
                        )
    ...
    def _tried_here_context(self, location: str,
                            current_state_snapshot: dict) -> str:
        """Merge all local attempt stores into one concise advisory block."""
        ...
        for entry in entries:
            marker = " *" if entry.get("exact_state") else ""
            outcomes = entry.get("outcomes", {}) or {}
            exhausted = (
                int(entry.get("attempts", 0)) >= 3
                and not any(
                    int(outcomes.get(kind, 0) or 0) > 0
                    for kind in ("state_change", "moved", "scored")
                )
            )
            tier = " [EXHAUSTED]" if exhausted else ""
            outcome = entry.get("outcome") or "outcome not recorded"
            lines.append(
                f"{entry.get('command', '')}{tier} | {outcome} | "
                f"x{max(1, int(entry.get('attempts', 0)))}{marker}"
            )
        return "\n".join(lines)
