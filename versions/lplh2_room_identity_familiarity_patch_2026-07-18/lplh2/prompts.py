...
- **Tried Here**: one combined list shows every command already attempted at this location with its outcome and attempt count. Commands marked with * were tried in the exact current state. Prefer commands not on the list. Retry a listed command only when you can name a concrete change, such as a new item, a changed observation, or a changed score, that justifies it.
...
7. Tried Here is one combined list of commands already attempted at this location with outcomes and attempt counts. Do not re-propose a listed command unless the observation, inventory, visible objects, or score changed enough to justify it. A verb that failed on one object can still work on another object.
