...
class LPLHAgent:
    ...
    def __init__(self, llm_client: LLMClient = None, fm_client: FmClient = None):
        ...
        # fm structured tasks used at runtime: validate / extract / split.
            ...
            if is_valid is True:
                try:
                    ...
                    self.action_space.store_action(
                        action_split.get("verb", ""),
                        action_split.get("objects", []),
                    )
                except Exception as split_error:
                    logger.warning(f"Action split/storage failed: {split_error}")
    ...
    def _extract_observation_room_title(self, observation: str) -> str:
        ...
        Uses text formatting only. It avoids command acknowledgements and parser
        errors, and returns an empty string when the first line is not title-like.
        """
        ...
        if lower_line.startswith(blocked_prefixes):
            # Room titles like "The Troll Room" are allowed below via the
            # explicit title regex; parser messages starting with "The ..." are
            # otherwise too easy to misread as rooms.
            if not re.match(
    ...
    def _run_situation_manager(self, auxiliary_gate: dict, action: str,
                               ...
                               inventory_before: set = None) -> dict:
        """Use one LLM call to manage enriched stored situations."""
        location = self.kg_map.current_location or "unknown"
        active_before = self.situation_memory.active_situations()
        gate_decision = (
        ...
        legacy_detection_decision = (
            auxiliary_gate.get("decision", {}).get("stored_situation_detection", {})
            if isinstance(auxiliary_gate, dict)
            else {}
        )
        visible_objects = self._visible_objects_for_location(location)
        ...
        manager_run = (gate_decision or {}).get("run")
        legacy_run = (legacy_detection_decision or {}).get("run")
        should_run = (
            True if manager_run is None and legacy_run is None
            else bool(manager_run) or bool(legacy_run)
        )
        if not should_run:
    ...
    def _normalize_manager_situation_list(self, value) -> list[dict]:
        ...
        for item in value:
            ...
            if situation.get("id") or (situation.get("location") and situation.get("situation")):
    ...
    def _normalize_manager_update_list(self, value) -> list[dict]:
        ...
        for item in value:
            if not isinstance(item, dict):
                continue
            if "to" in item or "from" in item:
                old = self._normalize_manager_situation(item.get("from", {}))
                new = self._normalize_manager_situation(item.get("to", {}))
                if old.get("id") and not new.get("id"):
                    new["id"] = old["id"]
                if new.get("id") or (new.get("location") and new.get("situation")):
                    result.append(new)
                continue
            situation = self._normalize_manager_situation(item)
            if situation.get("id") or (situation.get("location") and situation.get("situation")):
    ...
    def _normalize_manager_situation(self, item) -> dict:
        if not isinstance(item, dict):
            return {"id": "", "location": "", "situation": ""}
        normalized = {
            "id": self._clean_text(item.get("id", "")),
            "location": self._clean_text(item.get("location", "")),
            "situation": self._clean_text(item.get("situation", "")),
        }
        if "possible_solution" in item:
            normalized["possible_solution"] = self._clean_text(
                item.get("possible_solution", "")
            )
        for field in ("closest_existing_id", "covered_by_existing_id", "why_distinct"):
            if field in item:
                normalized[field] = self._clean_text(item.get(field, ""))
        return normalized
    ...
    def _apply_situation_manager_decision(self, decision: dict) -> dict:
        ...
        for situation in stored.get("add", []) or []:
            cleaned = self._normalize_manager_situation(situation)
            covered_by = cleaned.get("covered_by_existing_id", "").strip()
            if covered_by and covered_by.lower() != "none":
                active_ids = {item.get("id") for item in self.situation_memory.active_situations()}
                if covered_by in active_ids:
                    skipped = dict(cleaned)
                    skipped["reason"] = "llm_marked_duplicate_add"
                    result["skipped_adds"].append(skipped)
                    continue
