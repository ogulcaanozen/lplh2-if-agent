...
- **Retry Justification**: If you choose an exact command from Known Failed Commands Here or Same-State Tried Commands, the <rea> motivation must explicitly say what changed or why this retry is now useful. If you cannot name that reason, choose a different command.
...
1. **Priority Usage**
- **Highest Priority**: Current inventory and stored situations whose possible_solution is concretely relevant now.
...
- If you retry an exact failed/same-state command, include the concrete retry reason in <rea>; otherwise do not retry it.
...
2. Inventory reconciliation routing
   Decide whether the dedicated inventory reconciliation LLM should run. Do NOT
   add or remove items here; only decide if the semantic inventory reconciler
   should be called.
   - Set "run": true when the observation may imply carried inventory changed,
     confirms the player already carries an object, lists inventory, or reports
     that a carried item was dropped, consumed, transferred, stolen, lost, or
     otherwise no longer carried.
   - Set true for mixed observations where an object interaction both changes
     the world and may affect carried inventory.
   - Set false for ordinary room descriptions, navigation with no item effect,
     parser errors unrelated to possession, or vague loss/theft hints that do
     not identify any concrete item.
...
3. Situation manager routing
   ...
   - Set "run": true when the latest step may require situation-memory work:
     ...
     or a visible object, inventory item, learned command, route, or changed
     world state may suggest or refine a possible_solution for an active stored
     situation.
   ...
   - Set false for ordinary room descriptions, generic navigation, repeated
     ...
     interactions unrelated to any blocker, hazard, or stored situation.
...
{{
  ...
  "inventory_reconciliation": {{
    ...
    "focus": ["optional", "short", "item", "targets"]
  }},
  "situation_manager": {{
    "run": true or false,
    "reason": "short routing reason",
...
# ---------------------------------------------------------------------
# LPLH2 Enhancement: Situation Manager
# Maintains compact unresolved situations with possible-solution notes.
# ---------------------------------------------------------------------
...
Each active situation has this compact format:
{{
  "id": "stable id assigned by code",
  "location": "where the unresolved problem exists",
  "situation": "short description of the unresolved problem",
  "possible_solution": "short current best hypothesis, or empty string"
}}

You may do three things:
1. Add new stored situations.
2. Update existing stored situations, especially the possible_solution field.
...
in Current Inventory, possible_solution should say that it is visible but not
carried. If the useful object is carried, possible_solution should say that.
...
{{
  "stored_situations": {{
    "add": [
      {{
        "location": "where to remember/return",
        "situation": "short unresolved problem",
        "possible_solution": "short solution hypothesis or empty string",
        "closest_existing_id": "closest active situation id, or none",
        "covered_by_existing_id": "active id if this add is already covered, otherwise none",
        "why_distinct": "short reason this is genuinely separate, or empty if covered"
      }}
    ],
    "update": [
      {{
        "id": "copy exact id from Active Stored Situations",
        "location": "updated or same location",
        "situation": "updated or same situation",
        "possible_solution": "updated solution hypothesis or empty string"
      }}
    ],
    "remove": [
      {{"id": "copy exact id from Active Stored Situations", "reason": "why it is solved/obsolete"}}
    ]
  }}
}}
|end|
...
- Use "add" only for new unresolved problems, not ordinary objects or normal
  room descriptions.
- Before every add, compare the candidate with Active Stored Situations. If it
  concerns the same underlying object, obstacle, hazard, hidden feature, route
  problem, or future-return problem, update the existing situation instead of
  adding a duplicate.
- For every add, fill closest_existing_id, covered_by_existing_id, and
  why_distinct. Use "none" when there is no relevant active situation. If
  covered_by_existing_id is not "none", do not include that candidate in add;
  place it in update instead.
- Use "update" only when an existing situation remains active but its location,
  wording, or possible_solution should be rewritten.
- For update/remove, prefer the exact id from Active Stored Situations.
- Use "remove" only when the latest observation, inventory, score, or world
  change directly shows the stored situation is solved, superseded by a more
  specific active situation, permanently impossible, clearly irrelevant to the
  current world state, obsolete, or wrong. Do not remove merely because it is
  not actionable right now.
- Do not invent hidden game knowledge. Use only the provided state.
- Keep possible_solution as one concise sentence. It can be empty when no
  plausible solution is known yet.
- If a possible solution depends on an item that is visible but not carried,
  mention that it must be taken first.
- If a possible solution depends on a carried item, mention that it is carried
  and what kind of use may help.
- Do not recommend exact commands from Known Failed Commands Here or Same-State
  Tried Commands as possible_solution unless the observation, inventory,
  visible objects, score, or world state changed enough to justify retrying.
  Prefer broader wording such as "try another way to expose/move/open it" when
  a specific command already failed.

Good examples:

Example 1: new unresolved problem, no known solution yet
Current Location: Kitchen / upstairs
Observation After Action: It is pitch black. You are likely to be eaten by a grue.
Active Stored Situations: []
Output:
|start|
{{
  "stored_situations": {{
    "add": [
      {{
        ...
        "situation": "dark upstairs area may be dangerous without light",
        "possible_solution": "",
        "closest_existing_id": "none",
        "covered_by_existing_id": "none",
        "why_distinct": "No active stored situation covers this dark upstairs hazard."
      }}
    ],
    "update": [],
    "remove": []
...
Example 2: visible useful object not carried
Current Inventory: ["sword"]
Visible Objects Here: ["lantern"]
Active Stored Situations: [{{"id": "situation_001", "location": "Kitchen / upstairs", "situation": "dark upstairs area may be dangerous without light", "possible_solution": ""}}]
Output:
|start|
{{
  "stored_situations": {{
    ...
    "update": [
      {{
        ...
        "location": "Kitchen / upstairs",
        "situation": "dark upstairs area may be dangerous without light",
        "possible_solution": "The lantern visible here may help with the dark area, but it is not carried yet."
      }}
    ],
    "remove": []
  }}
...
Example 3: useful object now carried
Previous Action: get lantern
Observation After Action: Taken.
Current Inventory: ["sword", "lantern"]
Active Stored Situations: [{{"id": "situation_001", "location": "Kitchen / upstairs", "situation": "dark upstairs area may be dangerous without light", "possible_solution": "The lantern visible here may help with the dark area, but it is not carried yet."}}]
Output:
|start|
{{
  "stored_situations": {{
    ...
    "update": [
      {{
        ...
        "location": "Kitchen / upstairs",
        "situation": "dark upstairs area may be dangerous without light",
        "possible_solution": "The carried lantern may help; try turning it on before exploring the dark area."
      }}
    ],
    "remove": []
  }}
...
Example 4: no situation work needed
...
Example 5: observation refines an existing hidden-feature situation
Current Location: Living Room
Observation After Action: Underneath the rug is a closed trap door.
Active Stored Situations: [{{"id": "situation_003", "location": "Living Room", "situation": "irregularity beneath the rug may hide something important", "possible_solution": ""}}]
Output:
|start|
{{
  "stored_situations": {{
    ...
    "update": [
      {{
        ...
        "situation": "closed trap door beneath the rug may hide an important path or object",
        "possible_solution": "Try another way to move or expose the rug/trap door; avoid repeating exact failed rug commands unless something changes."
      }}
    ],
    "remove": []
  }}
...
3. Active stored situations, especially their possible_solution text, when the
   current room, inventory, or visible objects make them addressable.
...
11. Learned action-space context. Prefer known valid command templates when they fit current visible objects or inventory, while still allowing genuinely useful new verbs.
...
- You may suggest useful verbs even if they have not appeared in Learned Action Space.
...
- If you re-propose an exact failed or same-state command, the "reason" field must explicitly state what changed. If nothing changed, propose a different command.
...
Active Stored Situations: [{{"id": "situation_001", "location": "Kitchen / upstairs", "situation": "dark upstairs area may require light", "possible_solution": "The lantern visible in this room may help, but it is not carried yet."}}]
