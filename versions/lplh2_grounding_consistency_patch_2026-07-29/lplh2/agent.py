        ...
        try:
            ...
            if parse_error:
                ...
                if self.kg_map.location_uncertain:
                    ...
                    parsed["location"] = self._effective_location()
                    suffix = (
                        f" (about the unseen area entered via '{entered_via}' "
                        f"from {last_known})"
                    )
        ...
        if not self.kg_map.location_uncertain:
            ...
            if not destination:
                for transition_command, transition_destination in (
                    ...
                    if (
                        normalize_command_key(transition_command)
                        == command_key
                    ):
                        destination = str(transition_destination or "")
                        break
            if destination:
                destination_familiarity = self._room_familiarity_by_location().get(
                    destination,
                    {},
                )
                if destination_familiarity.get("tier") == "EXHAUSTED":
                    return (
                        f"the confirmed destination '{destination}' is "
                        "EXHAUSTED and has no current reward, route, or active-"
                        "goal reason that reopens it"
                    )
