...
Observation situations and preparation goals persist as learned knowledge
across epochs. Ordinary situations that are resolved are hidden for the rest
of the current epoch and become available again after the world resets.
"""
...
class SituationMemory:
    """Store persistent observations and preparation goals."""

    MAX_OPEN_GOALS = 5

    def __init__(self):
        self._situations: list[dict[str, Any]] = []
        self._keys: set[str] = set()
        self._resolved_this_epoch_keys: set[str] = set()
    ...
    def reset(self, full: bool = False):
        ...
        An ordinary epoch reset preserves learned situations but clears the
        epoch-local resolved set, because the game world has restarted. A full
        reset clears both situation stores and is used when experiences are not
        being preserved.
        """
        self._resolved_this_epoch_keys = set()
        self.last_add_status = ""
        if full:
            self._situations = []
            self._keys = set()
    ...
    def active_situations(
        self,
        inventory: list[str] | None = None,
        visible_objects: list | None = None,
    ) -> list[dict[str, str]]:
        output = [
            self._render_ordinary(item)
            for item in self._situations
            if self._key(item) not in self._resolved_this_epoch_keys
        ]
    ...
    def persistent_situations(self) -> list[dict[str, Any]]:
        """Return all learned ordinary situations, including epoch-suppressed ones."""
        return [dict(item) for item in self._situations]

    def persistent_situations_for_prompt(self) -> list[dict[str, str]]:
        """Return all learned situations without internal provenance fields."""
        return [self._render_ordinary(item) for item in self._situations]

    def resolved_this_epoch(self) -> list[dict[str, Any]]:
        """Return learned situations hidden after resolution this epoch."""
        return [
            dict(item)
            for item in self._situations
            if self._key(item) in self._resolved_this_epoch_keys
        ]
    ...
    def add(
        ...
        """Add a situation once and preserve its original creation provenance."""
        normalized = {
            ...
            "created_epoch": max(0, int(created_epoch or 0)),
            "created_step": max(0, int(created_step or 0)),
        }
        ...
        if key in self._keys:
            self.last_add_status = (
                "duplicate_resolved_this_epoch"
                if key in self._resolved_this_epoch_keys
                else "duplicate"
            )
            existing = next(
                (item for item in self._situations if self._key(item) == key),
                normalized,
            )
    ...
    def add_goal_situation(
        ...
        if existing:
            ...
            if item_keywords:
                existing["item_keywords"] = self._merge_clean_lists(
                    existing.get("item_keywords", []), item_keywords
                )
            if advice:
        ...
        goal = {
            ...
            "item_keywords": self._clean_list(item_keywords),
    ...
    def record_goal_survival(self, goal_id: str, inventory: Optional[list[str]],
                             epoch: int = 0, step: int = 0) -> dict[str, Any]:
        """Record an alive exit that did not verify the proposed preparation."""
        goal = self._goal_situations.get(goal_id)
        if not goal:
            return {"status": "not_found"}
        evidence = {
        ...
        survivals = goal.setdefault("survivals", [])
        if not survivals or survivals[-1] != evidence:
            survivals.append(evidence)
        return {
            "status": "survived_unprepared",
            "goal_id": goal_id,
    ...
    def reopen_goal(self, goal_id: str) -> dict[str, Any]:
        ...
        if not goal:
            return {"status": "not_found"}
        confirmed_inventory = self._clean_list(goal.get("confirmed_inventory", []))
        if confirmed_inventory:
            goal.setdefault("false_confirmations", []).append({
        ...
        return {
            "status": "reopened",
            "goal_id": goal_id,
            "false_confirmations": len(goal.get("false_confirmations", [])),
        }
    ...
    def update_goal_hypothesis(self, goal_id: str,
        ...
        if item_keywords is not None:
            goal["item_keywords"] = self._clean_list(item_keywords)
        if advice:
    ...
    def remove(self, situation: dict[str, str]) -> bool:
        """Suppress a solved situation until the next epoch reset."""
        key = self._key(situation)
        if key not in self._keys or key in self._resolved_this_epoch_keys:
            return False
        self._resolved_this_epoch_keys.add(key)
        return True
    ...
    @staticmethod
    def _render_ordinary(situation: dict[str, Any]) -> dict[str, str]:
        """Keep internal provenance out of gameplay prompts."""
        return {
            "location": str(situation.get("location", "") or ""),
            "situation": str(situation.get("situation", "") or ""),
            "possible_solution": str(
                situation.get("possible_solution", "") or ""
            ),
        }
    ...
    def _render_goal(
        ...
        if goal.get("status") == "avoid":
            ...
            keywords = {
                ...
                for value in goal.get("item_keywords", [])
                if self._normalize(value)
            }
            carried = self._matching_items(inventory, keywords)
            visible = self._matching_items(visible_objects, keywords)
            if keywords:
                solution += (
                    " Preparation evidence: recorded as possibly required: "
                    f"{', '.join(sorted(keywords))}; "
                    f"matching carried item(s): {', '.join(carried) or 'none'}; "
                    f"matching visible item(s): {', '.join(visible) or 'none'}."
                )
            if goal.get("survivals"):
                solution += " Retreating alive is possible; do not linger unprepared."
        return {
            "location": location,
            "situation": situation,
    ...
    def _matching_items(self, values: list | None, keywords: set[str]) -> list[str]:
        output = []
        keyword_tokens = {
            token
            ...
            if len(token) > 2
            and token not in {"other", "item", "thing", "suitable"}
        }
        for value in values or []:
            raw = value.get("name", "") if isinstance(value, dict) else value
            display = self._clean_field(raw)
            ...
            if (
                ...
                and (
                    ...
                    or bool(value_tokens & keyword_tokens)
                )
                and display not in output
            ):
