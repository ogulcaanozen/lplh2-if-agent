...
class FailedActionMemory:
    ...
    def record(self, location: str, command: str, observation: str,
               failure_reason: str, world_signature: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
        """Store a failure record if this location+command is not already known."""
        record = {
    ...
    def _location_key(self, location: str) -> str:
        return self._normalize(location)

    def _command_key(self, command: str) -> str:
        text = self._clean_command(command)
        directions = {
...
class StateScopedActionMemory:
    ...
    def _command_key(self, command: str) -> str:
        text = self._clean(command).lower()
        directions = {
            "n": "north", "s": "south", "e": "east", "w": "west",
            "ne": "northeast", "nw": "northwest", "se": "southeast", "sw": "southwest",
            "u": "up", "d": "down",
