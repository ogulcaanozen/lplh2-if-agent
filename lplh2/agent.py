...
class LPLHAgent:
    ...
    def act(self, observation: str, score: int, done: bool, info: dict,
        ...
        try:
            ...
            if is_valid:
                ...
                if prev_lower.startswith("take ") or prev_lower.startswith("get "):
                    item = completed_action[5:].strip() if prev_lower.startswith("take ") else completed_action[4:].strip()
                    self.kg_map.take_item(self._clean_inventory_item(item))
                elif prev_lower.startswith("drop "):
                    self.kg_map.drop_item(self._clean_inventory_item(completed_action[5:].strip()))
                elif prev_lower.startswith(("eat ", "drink ", "give ")):
                    item = completed_action.split(" ", 1)[1].strip() if " " in completed_action else ""
                    if item:
                        self.kg_map.consume_item(self._clean_inventory_item(item))
        ...
        if action_valid is True:
            ...
            stored, failure_record = self.failed_action_memory.record(
                location=failure_location,
                command=completed_action,
                observation=observation,
            ...
            action_failure_memory["status"] = "stored" if stored else "duplicate"
    ...
    def _generate_command(self, observation: str, score: int,
                          ...
                          reset_affordance_cache: bool = False) -> tuple:
